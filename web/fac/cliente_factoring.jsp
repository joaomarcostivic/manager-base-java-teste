...
function init()	{
    ...
    cdEmpresa = (<%=cdEmpresa%> != 0 ? <%=cdEmpresa%> : cdEmpresa);
...
function initPessoa(){
	...
	loadOptionsFromRsm($('cdClassificacaoCliente'), <%=sol.util.Jso.getStream(com.tivic.manager.adm.ClassificacaoClienteDAO.getAll())%>, {fieldValue: 'cd_classificacao_cliente', fieldText:'nm_classificacao_cliente'});
	if($('cdVinculo'))
...
// 	try { nrMoneyMask.attach($("vlLimiteFactoring"));} catch(e) {};
...
var columnsContaReceber = [{label:'Emitente', reference: 'NM_EMITENTE'},
...
function createGrid(content)	{
	if (content==null && $('cdPessoa').value != 0) {
		...
		getPage("GET", "createGrid", 
				...
				"&method="+method+"(const " + $('cdPessoa').value + ":int)", null, true);
	}
	else {
		var rsm = null;
...
function btnSavePessoaOnClick(content){
	if(content==null){
		if (disabledFormPessoa){
			...
			var stCadastro = ($('stCadastro').checked == true) ? 1 : 0;
...
// 			methodName += (cdFormularioDefault > 0 ? ", *atributos:java.util.ArrayList" : "");
			methodName += ", const "+ $('cdClassificacaoCliente').value +":int";
			methodName += ", vlLimiteFactoring:float, vlLimiteFactoringEmissor:float, vlLimiteFactoringUnitario:float, qtPrazoMinimoFactoring:int, qtPrazoMaximoFactoring:int, qtIdadeMinimaFactoring:int, vlGanhoMinimoFactoring:float, prTaxaMinimaFactoring:float, vlTaxaDevolucaoFactoring:float, prTaxaPadraoFactoring:float, prTaxaJurosFactoring:float, prTaxaProrrogacaoFactoring:float, qtMaximoDocumento:int)";
			if($("cdPessoa").value > 0)
...
function btnFindPessoaOnClick(reg, options){
    if(!reg){
        var filterFields = [[{label:"Nome", reference:"A.NM_PESSOA", datatype:_VARCHAR, comparator:_LIKE_ANY, width:50, charcase:'uppercase'},
                             {label:"Razão Social", reference:"NM_RAZAO_SOCIAL", datatype:_VARCHAR, comparator:_LIKE_ANY, width:50, charcase:'uppercase'}],
		...
		<% } %>
		$('vlLimiteFactoring').value 		  = formatCurrency(reg[0]['VL_LIMITE_FACTORING_TRABALHO']);
		$('vlLimiteFactoringEmissor').value   = formatCurrency(reg[0]['VL_LIMITE_FACTORING_EMISSOR_TRABALHO']);
		$('vlLimiteFactoringUnitario').value  = formatCurrency(reg[0]['VL_LIMITE_FACTORING_UNITARIO_TRABALHO']);
		...
		$('vlGanhoMinimoFactoring').value 	  = formatCurrency(reg[0]['VL_GANHO_MINIMO_FACTORING_TRABALHO']);
		$('prTaxaMinimaFactoring').value 	  = formatCurrency(reg[0]['PR_TAXA_MINIMA_FACTORING_TRABALHO']);
		$('vlTaxaDevolucaoFactoring').value   = formatCurrency(reg[0]['VL_TAXA_DEVOLUCAO_FACTORING_TRABALHO']);
		$('prTaxaPadraoFactoring').value 	  = formatCurrency(reg[0]['PR_TAXA_PADRAO_FACTORING_TRABALHO']);
		$('prTaxaJurosFactoring').value 	  = formatCurrency(reg[0]['PR_TAXA_JUROS_FACTORING_TRABALHO']);
		$('prTaxaProrrogacaoFactoring').value = formatCurrency(reg[0]['PR_TAXA_PRORROGACAO_FACTORING_TRABALHO']);
		$('qtMaximoDocumento').value 		  = reg[0]['QT_MAXIMO_DOCUMENTO_TRABALHO'];
		$('cdClassificacaoCliente').value = reg[0]['CD_CLASSIFICACAO_CLIENTE_TRABALHO'];
		...
		if(reg[0]['ST_CADASTRO'] == 0)
			$('stCadastro').checked = false;
		else if(reg[0]['ST_CADASTRO'] == 1)
			$('stCadastro').checked = true;
		/* VINCULOS */
		<%if(showDependentes)	{%>
...
function btnPrintPessoaOnClick(){
	parent.miRelatorioPessoaFactoringOnClick(true);
}
	...
	if ($('divGridEnderecos') != null) {
		if (content==null && $('cdPessoa').value != 0) {
			...
			if(rsmEnderecos != null){
				...
				$('cdCidadePessoaView').value = rsmEnderecos.lines[0]['NM_CIDADE'];
			}
...
function cdClassificacaoClienteOnChange(content, element)	{
	if (content == null) {
		if(element.value != 0){
			getPage("GET", "cdClassificacaoClienteOnChange", 
					"../methodcaller?className=com.tivic.manager.adm.ClassificacaoClienteDAO" + 
					"&method=get(const " + element.value + ":int)");
		}
	}
	else {
		var rsmClienteFactoring = null;
		try {rsmClienteFactoring = eval("(" + content + ")"); } catch(e) {}
		
		$('vlLimiteFactoring').value	 	 = formatCurrency(rsmClienteFactoring['vlLimiteFactoring']);
		$('vlLimiteFactoringEmissor').value	 = formatCurrency(rsmClienteFactoring['vlLimiteFactoringEmissor']);
		$('vlLimiteFactoringUnitario').value = formatCurrency(rsmClienteFactoring['vlLimiteFactoringUnitario']);
		$('qtPrazoMinimoFactoring').value	 = rsmClienteFactoring['qtPrazoMinimoFactoring'];
		$('qtPrazoMaximoFactoring').value	 = rsmClienteFactoring['qtPrazoMaximoFactoring'];
		$('qtIdadeMinimaFactoring').value	 = rsmClienteFactoring['qtIdadeMinimaFactoring'];
		$('vlGanhoMinimoFactoring').value	 = formatCurrency(rsmClienteFactoring['vlGanhoMinimoFactoring']);
		$('prTaxaMinimaFactoring').value	 = formatCurrency(rsmClienteFactoring['prTaxaMinimaFactoring']);
		$('vlTaxaDevolucaoFactoring').value	 = formatCurrency(rsmClienteFactoring['vlTaxaDevolucaoFactoring']);
		$('prTaxaJurosFactoring').value	 	 = formatCurrency(rsmClienteFactoring['prTaxaJurosFactoring']);
		$('prTaxaPadraoFactoring').value	 = formatCurrency(rsmClienteFactoring['prTaxaPadraoFactoring']);
		$('prTaxaProrrogacaoFactoring').value= formatCurrency(rsmClienteFactoring['prTaxaProrrogacaoFactoring']);
		$('qtMaximoDocumento').value	 	 = rsmClienteFactoring['qtMaximoDocumento'];
    ...
    <div id="divTabPessoa">
      <div id="divAbaDadosBasicos" style="display:none;">
        ...
        <div class="d1-line" id="line8">
          <div style="width: 251px;" class="element">
            <label class="caption" for="nmRazaoSocial">Razão social</label>
            <input style="text-transform: uppercase; width: 248px;" lguppercase="true" logmessage="Raz?o Social" class="field" idform="pessoa" reference="nm_razao_social" datatype="STRING" maxlength="50" id="nmRazaoSocial" name="nmRazaoSocial" type="text"/>
          </div>
          ...
          <div style="width: 115px;" class="element">
            ...
            <input style="text-transform: uppercase; width: 112px;" lguppercase="true" logmessage="Nº Inscrição Municipal" class="field" idform="pessoa" reference="nr_inscricao_municipal" datatype="STRING" maxlength="15" id="nrInscricaoMunicipal" name="nrInscricaoMunicipal" type="text"/>
          </div>
          <div style="width: 115px;" class="element">
            <label class="caption" for="nrInscricaoEstadual">Nº Inscrição Estadual</label>
            <input style="text-transform: uppercase; width: 115px;" lguppercase="true" logmessage="Nº Inscrição Estadual" class="field" idform="pessoa" reference="nr_inscricao_estadual" datatype="STRING" maxlength="15" id="nrInscricaoEstadual" name="nrInscricaoEstadual" type="text"/>
          </div>
        </div>
      ...
      <!--**************************** V-NCULOS/DADOS FUNCIONAIS ****************************-->
              ...
              <div id="divAbaFactoring" style="display:none; margin-left:3px;">
      			<fieldset style="font-family:Geneva, Arial, Helvetica, sans-serif; font-size:10px; float:left; width: 657px; height: 72px;">
					<legend>Opções</legend>
					<div style="width: 120px; margin-top: 10px;" class="element">
						<label class="caption" for="cdClassificacaoCliente">Tipos de Cliente</label>
						<select logmessage="Tipos de Cliente" style="width: 120px;" class="select2" idform="pessoa" reference="cd_classificacao_cliente" id="cdClassificacaoCliente" name="cdClassificacaoCliente" dataype="INTEGER"  onchange="cdClassificacaoClienteOnChange(null, this)">
							<option value="0">Nenhum</option>
						</select>
					</div>
					<div style="width: 22px; margin-top: 20px; margin-left: 450px;" class="element" id="checkbox">
		                <input style="width: 20px; font-size: 5px;" logmessage="Bloquear?" idform="pessoa" reference="st_cadastro" class="field2" name="stCadastro" id="stCadastro" type="checkbox" />
		            </div>
		            <div style="margin-top: 21px;" class="element" id="divcb" >
		            	<label class="caption" style="width: 50px; font-size: 12px;"id="labelcheckbox" >Bloquear</label>
		            </div>
		        </fieldset>
      			<fieldset style="font-family:Geneva, Arial, Helvetica, sans-serif; font-size:10px; float:left; width: 658px; height: 72px;">
            	  <legend>Taxas</legend>
		 			<div id="line8" class="d1-line" style="float: left;">
		 				<div style="width: 130px;  margin-top: 10px;" class="element">
							<label class="caption">Taxa de Devolução (R$)</label>
							<input style="width: 125px;" id="vlTaxaDevolucaoFactoring" reference="vl_taxa_devolucao_factoring" idform="pessoa" datatype="FLOAT" mask="#,####.00" defaultValue="0,00" name="vlTaxaDevolucaoFactoring" class="field2"  type="text"/>
				 		</div>
						<div style="width: 130px;  margin-top: 10px;" class="element">
							<label class="caption">Taxa Padrão (%)</label>
							<input style="width: 125px;" id="prTaxaPadraoFactoring" reference="pr_taxa_padrao_factoring" onblur="addParametros(this)" idform="pessoa" datatype="FLOAT" mask="#,####.00" defaultValue="0,00" name="prTaxaPadraoFactoring" class="field2" type="text"/>
				 		</div>
						<div style="width: 130px;  margin-top: 10px;" class="element">
							<label class="caption">Taxa de Juros (%)</label>
							<input style="width: 125px;" id="prTaxaJurosFactoring" reference="pr_taxa_juros_factoring" onblur="addParametros(this)" idform="pessoa" datatype="float" mask="#,####.00" defaultValue="0,00" name="prTaxaJurosFactoring" class="field2"  type="text"/>
				 		</div>
				 		<div style="width: 130px;  margin-top: 10px;" class="element">
							<label class="caption">Taxa de Prorrogação (%)</label>
							<input style="width: 125px;" id="prTaxaProrrogacaoFactoring" reference="pr_taxa_prorrogacao_factoring" onblur="addParametros(this)" idform="pessoa" datatype="float" mask="#,####.00" defaultValue="0,00" name="prTaxaProrrogacaoFactoring" class="field2" type="text"/>
				 		</div>
						<div style="width: 138px;  margin-top: 10px;" class="element">
							<label class="caption">Taxa Minima (%)</label>
							<input style="width: 138px;" id="prTaxaMinimaFactoring" reference="pr_taxa_minima_factoring" onblur="addParametros(this)" idform="pessoa" datatype="float" mask="#,####.00" defaultValue="0,00" name="prTaxaMinimaFactoring" class="field2" type="text"/>
				 		</div>
		 			</div>
				</fieldset>
              	<fieldset style="font-family:Geneva, Arial, Helvetica, sans-serif; font-size:10px; float:left; width: 368px; height: 72px;">
            	  <legend>Limite de crédito</legend>
		 			<div id="line8" class="d1-line" style="float: left;">
		 				<div style="width: 122px;  margin-top: 10px;" class="element">
							<label class="caption">Limite Geral (R$)</label>
							<input style="width: 117px;" id="vlLimiteFactoring" reference="vl_limite_factoring" onblur="addParametros(this)" idform="pessoa" datatype="FLOAT" mask="#,####.00" defaultValue="0,00" name="vlLimiteFactoring" class="field2"  type="text"/>
				 		</div>
						<div style="width: 122px;  margin-top: 10px;" class="element">
							<label class="caption">Limite Emissor (R$)</label>
							<input style="width: 117px;" id="vlLimiteFactoringEmissor" reference="vl_limite_factoring_emissor" onblur="addParametros(this)" idform="pessoa" datatype="FLOAT" defaultValue="0,00" mask="#,####.00" name="vlLimiteFactoringEmissor" class="field2" type="text"/>
				 		</div>
						<div style="width: 122px;  margin-top: 10px;" class="element">
							<label class="caption">Limite Unitario (R$)</label>
							<input style="width: 122px;" id="vlLimiteFactoringUnitario" reference="vl_limite_factoring_unitario" onblur="addParametros(this)" idform="pessoa" datatype="FLOAT" mask="#,####.00" defaultValue="0,00" name="vlLimiteFactoringUnitario" class="field2"  type="text"/>
				 		</div>
		 			</div>
				</fieldset>
				<fieldset style="font-family:Geneva, Arial, Helvetica, sans-serif; font-size:10px; float:left; width: 270px; height: 72px;">
            	  <legend>Prazos</legend>
		 			<div id="line8" class="d1-line" style="float: left;">
		 				<div style="width: 130px;  margin-top: 10px;" class="element">
							<label class="caption">Minimo (Meses)</label>
						...
						<div style="width: 138px;  margin-top: 10px;" class="element">
							<label class="caption">Maximo (Meses)</label>
							<input style="width: 138px;" id="qtPrazoMaximoFactoring" reference="qt_prazo_maximo_factoring" onblur="addParametros(this)" idform="pessoa" datatype="float" mask="#,####.00" name="qtPrazoMaximoFactoring" class="field2" type="text"/>
				 		</div>
					</div>
				</fieldset>
				<fieldset style="font-family:Geneva, Arial, Helvetica, sans-serif; font-size:10px; float:left; width: 657px; height: 72px;">
            	  <legend>Outros</legend>
					<div style="width: 219px;  margin-top: 10px;" class="element">
						<label class="caption">Idade Minima (Meses)</label>
						<input style="width: 214px;" id="qtIdadeMinimaFactoring" reference="qt_idade_minima_factoring" onblur="addParametros(this)" idform="pessoa" datatype="float" name="qtIdadeMinimaFactoring" class="field2" type="text"/>
			 		</div>
			 		<div style="width: 219px;  margin-top: 10px;" class="element">
						<label class="caption">Ganho Minimo (R$)</label>
						<input style="width: 214px;" id="vlGanhoMinimoFactoring" reference="vl_ganho_minimo_factoring" onblur="addParametros(this)" idform="pessoa" datatype="float" mask="#,####.00" defaultValue="0,00" name="vlGanhoMinimoFactoring" class="field2" type="text"/>
			 		</div>
			 		<div style="width: 219px;  margin-top: 10px;" class="element">
						<label class="caption">Nº máximo de documentos abertos</label>
						<input style="width: 219px;" id="qtMaximoDocumento" reference="qt_maximo_documento" onblur="addParametros(this)" idform="pessoa" datatype="int" name="qtMaximoDocumento" class="field2" type="text"/>
			 		</div>
		 		</fieldset>
			  </div>
			  <div id="divAbaChequesAberto" style="display:none; margin-left:3px;">
			  	<div id="divGridContaReceber" style="width: 675px; background-color:#FFF; height:345px; border:1px solid #000;">&nbsp;</div>
              </div>
            </div>
          </div>
    </div>

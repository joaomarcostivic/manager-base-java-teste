...
function getAllNivelLocal(content)	{
	if (content==null) {
		...
		tvNivelLocal = TreeOne.create('tvNivelLocal', {width: 534, height: 142, resultset: niveisLocais, columns: ['NM_NIVEL_LOCAL'],
										               defaultImage: '../alm/imagens/nivel_local_tv16.gif', plotPlace: document.getElementById('divTreeNivelLocal'),
										               onSelect: onTreeviewNivelLocalOnClick});
		for (var i=0; niveisLocais!=null && i<niveisLocais.lines.length; i++)
			addNivelLocal(niveisLocais.lines[i]);
	}
}

function addNivelLocal(setor)	{
	var option = document.createElement('OPTION');
	option.setAttribute('value', setor['CD_NIVEL_LOCAL']);
	option.appendChild(document.createTextNode(setor['NM_NIVEL_LOCAL']));
	...
	if(subNivelLocales != null){
		for(var i=0;i<subNivelLocales.lines.length; i++)
			addNivelLocal(subNivelLocales.lines[i]);
	}
}
...
<div style="width: 346px;" id="setor" class="d1-form">
  <input logmessage="Setor"  idform="nivelLocal" reference="lg_setor" id="lgSetor" name="lgSetor" type="hidden" value="1"/>
  ...
  <div style="width: 346px; height: 305px;" class="d1-body">
    ...
    <div class="d1-line" id="line0">
      <div style="width: 240px;" class="element">
        ...
        <input style="text-transform: uppercase; width: 237px;" lguppercase="true" logmessage="Nome Nível Local" class="field" idform="nivelLocal" reference="nm_nivel_local" datatype="STRING" maxlength="50" id="nmNivelLocal" name="nmNivelLocal" type="text"/>
      </div>
	 ...
	 <div style="width: 80px;" class="element">
        ...
        <label style="margin:3px 0px 0px 0px" class="caption">Possui Unidades</label>
     </div>
    </div>

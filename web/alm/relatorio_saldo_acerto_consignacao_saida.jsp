...
var rsmAcertos = null;
var columnsAcerto = [{label:'Nome', reference:'NM_CLIENTE'},
...
function btPesquisarOnClick(content) {
	if (content==null) {
		...
		if (document.getElementById('cdFabricanteSearch').value!=0) {
			objects += 'item1=sol.dao.ItemComparator(const A.cd_fabricante:String, const ' + document.getElementById('cdFabricanteSearch').value + ':String, const <%=ItemComparator.EQUAL%>:int, const <%=java.sql.Types.INTEGER%>:int);';
			execute += 'criterios.add(*item1:java.lang.Object);';
		}
		/* produtos ativos */
		if (document.getElementById('stProdutoServicoSearch').checked) {
			objects += 'item2=sol.dao.ItemComparator(const D.st_produto_empresa:String, const 1:String, const <%=ItemComparator.EQUAL%>:int, const <%=java.sql.Types.INTEGER%>:int);';
		...
		/* fornecedor */
		if (document.getElementById('cdClienteSearch').value!=0) {
			objects += 'item4=sol.dao.ItemComparator(const cdCliente:String, const ' + document.getElementById('cdClienteSearch').value + ':String, const <%=ItemComparator.EQUAL%>:int, const <%=java.sql.Types.INTEGER%>:int);';
		...
		getPage('GET', 'btPesquisarOnClick', 
				...
				'&method=findSaldoConsignacao(const ' + cdEmpresa + ':int, const 0:int, const <%=com.tivic.manager.alm.ProdutoEstoqueServices.CONSIG_SAIDA%>:int, *criterios:java.util.ArrayList):int');
	}
	else {
		rsmAcertos = null;
...
function createGrid(rsm)	{
	gridAcertos = GridOne.create('gridAcertos', {columns: columnsAcerto,
														 resultset: rsm,
														 plotPlace: document.getElementById('divGridAcertos'),
...
function btImprimirOnClick(content) {
	if (rsmAcertos != null) {
		...
		parent.ReportOne.create('jReportSaldoAcertosSaidaTemp', {width: 590,
										 height: 350,
										 caption: 'Relatório de Saldos de Consignação (Clientes) a Acertar',
										 resultset: rsmAcertos,
										 pageHeaderBand: {defaultImage: urlLogo,
										 				defaultTitle: 'Relatório de Saldos de Consignação (Clientes) a Acertar',
...
function btnFindCdClienteOnClick(reg){
    if(!reg){
		...
		filterWindow.close();
		document.getElementById('cdClienteSearch').value = reg[0]['CD_PESSOA'];
		document.getElementById('nmClienteViewSearch').value = reg[0]['NM_PESSOA'];
	}
}

function btnClearCdClienteOnClick(){
	document.getElementById('cdClienteSearch').value = 0;
	document.getElementById('nmClienteViewSearch').value = '';
}
</script>
</head>
   ...
   <div style="width: 580px; height: 500px;" class="d1-body">
	 <div class="d1-line" id="line0">
		...
		<div style="width: 250px;" class="element">
			<label class="caption" for="cdClienteSearch">Cliente</label>
			<input datatype="STRING" id="cdClienteSearch" name="cdClienteSearch" type="hidden" value="0" defaultValue="0">
			<input style="width: 247px;" static="true" class="disabledField" disabled="disabled" name="nmClienteViewSearch" id="nmClienteViewSearch" type="text">
			<button onclick="btnFindCdClienteOnClick()" idform="produtoServico" title="Pesquisar valor para este campo..." class="controlButton controlButton2"><img alt="L" src="/sol/imagens/filter-button.gif"></button>
			<button onclick="btnClearCdClienteOnClick()" idform="produtoServico" title="Limpar este campo..." class="controlButton"><img alt="X" src="/sol/imagens/clear-button.gif"></button>
		  </div>

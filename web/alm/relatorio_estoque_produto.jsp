...
<loader:library libraries="form, toolbar, calendario, aba2.0, filter, grid2.0, report, shortcut, report, jquery, util" compress="false"/>
<%
	try {
...
function init()	{
	...
	grupoCombustiveis = <%=Jso.getStream(GrupoServices.getAllCombustivelAsArray(cdEmpresa, null))%>;
...
function btnVisualizarOnClick()	{
    ...
    nomeJasper = "consolidacao_estoque_produtos_simples";
	method     = "gerarRelatorioEstoqueProduto(const "+$('cdEmpresa').value+":int, *crt:java.util.ArrayList())";
	listaSubReports = "alm/estoque_produtos_subreport_produtos_referencia";
    
	var arLocalArmazenamento = new Array();
	var arDsLocalArmazenamento = new Array();
	...
	for( var i=0; i < rsmOptionsLocaisEstoque.lines.length;i++ ){
		if( rsmOptionsLocaisEstoque.lines[i].CHECKED == 1 ){
			...
			arDsLocalArmazenamento.push( rsmOptionsLocaisEstoque.lines[i].LABEL );
		}
	}
	...
	for( var i=0; i < rsmColumnsSubCatProdutos.lines.length;i++ ){
		if( rsmColumnsSubCatProdutos.lines[i].CHECKED == 1 ){
			arCategoria.push( rsmColumnsSubCatProdutos.lines[i].CD_GRUPO );		
		}
	}
	...
	if( arCategoria.length > 0 ){
		objects += 'cat=sol.dao.ItemComparator(const H.cd_grupo:String,const '+arCategoria.join('|')+':String, const '+_IN+':int, const '+_INTEGER+':int);';
		execute += 'crt.add(*cat:Object);';
	}
	//ELIMINA OS COMBUSTÍVEIS DO RESULTADO
	objects += 'comb=sol.dao.ItemComparator(const H.cd_grupo:String,const '+grupoCombustiveis.join("|")+':String, const '+_NOTIN+':int, const '+_INTEGER+':int);';
	execute += 'crt.add(*comb:Object);';
	
	objects+='dtEstoq=sol.dao.ItemComparator(const dt_movimento:String,const '+ $('dtEstoque').value+':String, const '+_EQUAL+':int, const '+_DATE+':int);';
	execute += 'crt.add(*dtEstoq:Object);';
	
	var cdFabricante = $('cdFabricanteSearch').value;
	if(cdFabricante != null && cdFabricante != undefined && cdFabricante != 0){
		objects+='cdFabric=sol.dao.ItemComparator(const A.cd_fabricante:String,const '+ cdFabricante +':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
		execute += 'crt.add(*cdFabric:Object);';
	}

	var tpEstoque = $('tpEstoque').value;
	if(tpEstoque != -1){
		objects+='tpEstoq=sol.dao.ItemComparator(const tpEstoque:String,const '+ tpEstoque +':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
		execute += 'crt.add(*tpEstoq:Object);';
	}
	
	var tpCtrlEstoque = $('tpControleEstoque').value;
	if(tpCtrlEstoque != -1){
		objects+='tpCtrlEstoq=sol.dao.ItemComparator(const B.tp_controle_estoque:String,const '+ tpCtrlEstoque +':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
		execute += 'crt.add(*tpCtrlEstoq:Object);';
	}
	...
	if(cdProduto != null & cdProduto != undefined && cdProduto != 0){
		objects+='cdProd=sol.dao.ItemComparator(const A.cd_produto_servico:String,const '+ cdProduto+':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
		execute += 'crt.add(*cdProd:Object);';
	}
	
	var cor = $('txtEspecificacao').value;
	if(cor != null && cor != undefined && cor != ''){
		objects+='txtEspec=sol.dao.ItemComparator(const txt_especificacao:String,const '+ cor +':String, const '+_LIKE_ANY+':int, const '+_VARCHAR+':int);';
		execute += 'crt.add(*txtEspec:Object);';
	}
	
	var numeracao = $('txtDadoTecnico').value;
	if(numeracao != null && numeracao != undefined && numeracao != ''){
		objects+='txtDadoTec=sol.dao.ItemComparator(const txt_dado_tecnico:String,const '+ numeracao +':String, const '+_LIKE_ANY+':int, const '+_VARCHAR+':int);';
		execute += 'crt.add(*txtDadoTec:Object);';
	}
	...
	if(sit != null && sit != undefined && sit != -1  ){
		objects+='sit=sol.dao.ItemComparator(const B.st_produto_empresa:String,const '+ parseInt(sit) +':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
	...
	var imp = jQuery('input[name="impressaoPor"]:checked').val();
	objects+='impPor=sol.dao.ItemComparator(const impressaoPor:String,const '+ (imp != undefined ? parseInt(imp) : -1)+':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
	execute += 'crt.add(*impPor:Object);';
	
	var sub = $('tpSubcategoria').checked ? "1" : "0";
	params += 'LG_SUB_CATEGORIA|'+String( ( sub == 1 ) );
	
	var col = $('tpColunaVendaAVista').checked ? "0" : "1";
	objects+='tpVendaVista=sol.dao.ItemComparator(const lgColunaVendaAVista:String,const '+ col +':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
	execute += 'crt.add(*tpVendaVista:Object);';
	
	var colMargem = $('tpColunaMargem').checked ? "0" : "1";
	objects+='tpColunaMargem=sol.dao.ItemComparator(const lgColunaMargem:String,const '+ colMargem +':String, const '+_EQUAL+':int, const '+_INTEGER+':int);';
	execute += 'crt.add(*tpColunaMargem:Object);';
	
	var dsLocaisArmazenamento = "Todos";
	if( arDsLocalArmazenamento.length > 0 )
		dsLocaisArmazenamento = arDsLocalArmazenamento.join(", ");
			
	params += "-=-DS_LOCAIS_ARMAZENAMENTO|"+dsLocaisArmazenamento;
...
function btnFindFabricanteOnClick(reg){
    if(!reg){
		FilterOne.create("jFiltro", {caption:"Pesquisar Fabricantes", width: 350, height: 225,  modal: true,
...
function createLocaisEstoqueOptions()	{
	...
	gridLocaisEstoque = GridOne.create('gridLocaisEstoque', {plotPlace: $('divGridLocaisEstoque'), 
...
<div style="width: 892px;" id="relatorioProduto" class="d1-form">
   <div style="width: 892px; height: 457px;" class="d1-body">
	 ...
	 <div id="toolBar" class="d1-toolBar" style="height:113px; width: 200px; float: left;"></div>
   	 <div style="border:1px solid #999; padding:2px 2px 2px 4px; height: 108px; margin-bottom: 2px; width:650px; float: left; margin-left: 5px;">
		 <div class="d1-line" id="line0">
			<div style="width: 89px;" class="element">
	            ...
	            <label class="caption" for="cdFornecedor">Fabricante</label>
	            <input idform="relFabricante" nullvalue="0" column="cdFabricante" relation="<%=ItemComparator.EQUAL%>" sqltype="<%=Types.INTEGER%>" datatype="STRING" id="cdFabricanteSearch" name="cdFabricanteSearch" type="hidden" value="0" defaultValue="0"/>
	            <input style="width: 316px;" static="true" class="disabledField2" disabled="disabled" name="nmFabricanteViewSearch" id="nmFabricanteViewSearch" type="text"/>
	            <button onclick="btnFindFabricanteOnClick()" idform="relFabricante" style="height: 22px;" title="Pesquisar valor para este campo..." class="controlButton controlButton2"><img alt="L" src="/sol/imagens/filter-button.gif"/></button>
	            <button onclick="btnClearFabricanteOnClick()" idform="relFabricante" style="height: 22px;" title="Limpar este campo..." class="controlButton"><img alt="X" src="/sol/imagens/clear-button.gif"/></button>
          </div>
		...
		<div class="d1-line" id="line0">
          <div style="width: 330px;" class="element">
	            ...
	            <input style="width: 296px;" static="true" class="disabledField2" disabled="disabled" name="nmFornecedorViewSearch" id="nmFornecedorViewSearch" type="text"/>
		    ...
		    <select nullvalue="-1" column="stPreco" relation="<%=ItemComparator.EQUAL%>" sqltype="<%=Types.INTEGER%>" idform="relProduto" style="width: 145px;" class="select2" datatype="STRING" id="stPreco" name="stPreco">
		    ...
		    <select style="width: 144px;" nullvalue="-1" column="cdTabelaPreco" relation="<%=ItemComparator.EQUAL%>" sqltype="<%=Types.INTEGER%>" idform="relProduto" class="select2" datatype="STRING" id="cdTabelaPreco" name="cdTabelaPreco">
		    </select>
       	  </div>
     	</div>
		<div class="d1-line" id="line0">
			<div style="width: 330px;" class="element">
	            <label class="caption" for="cdProduto">Produto</label>
	            <input idform="relProduto" nullvalue="0" column="cdFornecedor" relation="<%=ItemComparator.EQUAL%>" sqltype="<%=Types.INTEGER%>" datatype="STRING" id="cdProdutoSearch" name="cdProdutoSearch" type="hidden" value="0" defaultValue="0"/>
	            <input style="width: 296px;" static="true" class="disabledField2" disabled="disabled" name="nmProdutoViewSearch" id="nmProdutoViewSearch" type="text"/>
	            <button onclick="btnFindProdutoOnClick()" idform="relProduto" style="height: 22px;" title="Pesquisar valor para este campo..." class="controlButton controlButton2"><img alt="L" src="/sol/imagens/filter-button.gif"/></button>
	            <button onclick="btnClearProdutoOnClick()" idform="relProduto" style="height: 22px;" title="Limpar este campo..." class="controlButton"><img alt="X" src="/sol/imagens/clear-button.gif"/></button>
          	</div>
       	  	<div style="width: 95px; margin-left: 110px;" class="element">
    ...
    <div class="d1-line" id="line0" style="height: 250px; float: left;">
	  	<div style="width: 187px;" class="element">
			<label class="caption" for="divGridCategoria">Categoria</label>
     		<div id="divGridCategoria" style="width:180px; height:250px; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element"></div>
   		</div>
	  	<div style="width: 187px;" class="element">
		  <label class="caption" for="divGridSubcategoria">SubCategoria</label>
          <div id="divGridSubcategoria" style="width:180px; height:250px; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element"></div>
   		</div>
   		<div style="width: 250px;" class="element">
   			<label class="caption" for="calculoCusto">Cálculo do Custo</label>
   			<div id="calculoCusto" style="width:200px; height:130; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element">
	   			<input type="radio" name="calculoCusto" value="0" checked="checked">Último Custo
	   			<input type="radio" name="calculoCusto" value="1"  >Custo Médio
   			</div>
   			<label class="caption" for="comSemEstoque">Produtos com ou sem estoque</label>
   			<div id="comSemEstoque" style="width:200px; height:130; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element">
	   			<input type="radio" name="comSemEstoque" value="-1" checked="checked">Todos
	   			<input type="radio" name="comSemEstoque" value="0">Produtos sem estoque
	   			<input type="radio" name="comSemEstoque" value="1">Produtos com estoque
   			</div>
   			<label class="caption" for="situacao">Situação</label>
   			<div id="situacao" style="width:200px; height:130; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element">
	   			<input type="radio" name="situacao" value="-1" checked="checked">Todos
	   			<input type="radio" name="situacao" value="0">Inativo
	   			<input type="radio" name="situacao" value="1">Ativo
   			</div>
   			<div style="width:200px; height:130; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element">
	   			<input type="checkbox" id="tpSubcategoria" name="tpSubcategoria" />Inclui nome de subcategoria
	   			<br />
	   			<input type="checkbox" id="tpColunaVendaAVista" name="tpColunaVendaAVista" />Não mostrar coluna venda a vista
	   			<br />
	   			<input type="checkbox" id="tpColunaMargem" name="tpColunaMargem" />Não mostrar coluna margem
   			</div>
   		</div>
   		<div style="width: 250px;" class="element">
			<label class="caption" for="divGridLocaisEstoque">Locais de Estoque</label>
		    <div id="divGridLocaisEstoque" style="width:240px; height:223px; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element">
	        </div>
	        <div style="display: inline-block;">
	          	<label class="caption" for="ordemImpressao" style="width: 100px;">Ordem Impressão</label>
				<div id="ordemImpressao" style="width:100px; height:130; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element">
		 			<input type="radio" name="ordemImpressao" value="0">Código
		 			<br />
		 			<input type="radio" name="ordemImpressao" value="1">Descrição
				</div>
	        </div>
<!-- 	        <div style="display: inline-block;"> -->
<!-- 	          	<label class="caption" for="impressaoPor">Impressão por</label> -->
<!-- 				<div id="impressaoPor" style="width:100px; height:130; border:1px solid #CCC; margin:2px 1px 0 0; background-color:#FFF;" class="element"> -->
<!-- 		 			<input type="radio" name="impressaoPor" value="0">Categoria -->
<!-- 		 			<br /> -->
<!-- 		 			<input type="radio" name="impressaoPor" value="0">Alíquota -->
<!-- 				</div> -->
<!-- 			</div> -->
   		</div>
   	</div>

...
<%
	...
	String nrInscricaoImobiliaria = RequestUtilities.getAsString(request, "nrInscricaoImobiliaria", "");
	String output = "";
	int nrAnoBase = com.tivic.manager.grl.ParametroServices.getValorOfParametroAsInteger("NR_ANO_BASE", 2014, 0);

	try {
		Result result = com.tivic.manager.egov.ITBIServices.getDAM(nrInscricaoImobiliaria);
		if(result.getCode()<0)	{
			...
			String[] nrCodigoBarras    = com.tivic.manager.egov.ITBIServices.getCodigoBarras(nrDAM, vlParcela, dtVencimento, nrParcela);
			//
			...
			JasperPrint jp = JasperFillManager.fillReport(path+"/itbi.jasper", parametros, jrRS);
		...
		output = "Erro ao processar DAM/ITBI";
		response.reset();
		response.setContentType("text/html");
	}
%>
<html>
<head>
<title>ITBI <%=nrAnoBase%> - Documento de Arrecadação Municipal</title>
</head>

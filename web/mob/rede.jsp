...
<%@page import="sol.util.RequestUtilities"%>
 <%
  ...
  %>
<head>
<title>Rede de Equipamentos</title>
...
<script language="javascript">
	...
	var tpExibicaoGeotracker = 0;
	var cdCidade = 0;
	
	var url = 'http://tivic.com.br/etransito'; //'..'
	...
	function init() {
		...
		if(Session.isLogged()) {
			...
			cdCidade = Session.getParam('CD_CIDADE_END_DEFAULT');
			...
			$("#idOrgao").change(function() {
				...
				if (tpExibicaoGeotracker==1){
					loadEquipamentosVeiculo();
				}
			});

			loadOrgaos();
			
			$('#nmCidade').val(cdCidade > 0 ? cdCidade : 'TODAS');
			
			$('#dtInicial').val(formatDate(hoje));
			$('#dtFinal').val(formatDate(hoje));
			
			$("#nmUsuario").val(authData.pessoa.nmPessoa);
	...
	function loadOrgaos() {
		$.ajax({
			url : url+'/methodcaller?className=com.tivic.manager.str.OrgaoServices&method=getAllByCidade(const '+ cdCidade + ':int)',
			dataType: 'text',
			success: function(content) {
				var rsm = null;
				try { rsm = eval('(' + content + ')'); } catch (e) { }
				
				loadFromRsm("#idOrgao", rsm, "ID_ORGAO", "NM_ORGAO", true);
				
				if(cdCidade > 0 && rsm.lines.length > 0) {
	...
	function loadAgentes() {
		$.ajax({
			url : url+'/methodcaller?className=com.tivic.manager.str.AgenteServices&method=getAllByIdOrgao(const '+ $('#idOrgao').val() + ':String)',
			dataType: 'text',
			success: function(content) {
				var rsm = null;
				try { rsm = eval('(' + content + ')'); } catch (e) { }
	...
	function loadEquipamentosVeiculo() {
		$.ajax({
			url : url+'/methodcaller?className=com.tivic.manager.mob.VeiculoEquipamentoServices&method=getAllByIdOrgao(const '+ $('#idOrgao').val() + ':String)',
			dataType: 'text',
			success: function(content) {
				var rsm = null;
				try { rsm = eval('(' + content + ')'); } catch (e) { }
	...
	function trackAgentes() {
		...
		$.ajax({
			url : url+"/methodcaller?className=com.tivic.manager.str.GeoTrackerServices" +
...
<div class="accordion with-marker" data-role="accordion" data-closeany="true" style="position: absolute; left: 5px; top: 5px; width: 250px;">
	...
	<div class="accordion-frame">
	    ...
	    <div class="content">
			...
			<div class="input-control text">
			    <input name="nmCidade" id="nmCidade" type="text" disabled/>
			</div>
			<lable>Orgão</lable>
...
<div id="panelAgentes" class="panel" data-role="panel" style="position:absolute; left:290px; top:70px; display:none;">
...
<div id="panelVeiculos" class="panel" data-role="panel" style="position:absolute; left:290px; top:70px; display:none;">

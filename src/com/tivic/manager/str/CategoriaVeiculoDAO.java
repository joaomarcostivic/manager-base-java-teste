...
public class CategoriaVeiculoDAO{
	...
	public static int insert(CategoriaVeiculo objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("str_categoria_veiculo", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO str_categoria_veiculo (cd_categoria,"+
			                                  "nm_categoria) VALUES (?, ?)");
			pstmt.setInt(1, code);
	...
	public static int update(CategoriaVeiculo objeto, int cdCategoriaOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE str_categoria_veiculo SET cd_categoria=?,"+
	...
	public static int delete(int cdCategoria, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM str_categoria_veiculo WHERE cd_categoria=?");
	...
	public static CategoriaVeiculo get(int cdCategoria, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM str_categoria_veiculo WHERE cd_categoria=?");
			pstmt.setInt(1, cdCategoria);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM str_categoria_veiculo");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM str_categoria_veiculo", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

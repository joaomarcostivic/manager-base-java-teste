...
public class ProcessoDAO{
	...
	public static int insert(Processo objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("str_processo", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO str_processo (cd_processo,"+
			                                  "cd_ait,"+
			                                  "nr_processo,"+
			                                  "dt_processo,"+
			                                  "ds_parecer,"+
	...
	public static int update(Processo objeto, int cdProcessoOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE str_processo SET cd_processo=?,"+
												      		   "cd_ait=?,"+
												      		   "nr_processo=?,"+
												      		   "dt_processo=?,"+
												      		   "ds_parecer=?,"+
	...
	public static int delete(int cdProcesso, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM str_processo WHERE cd_processo=?");
	...
	public static Processo get(int cdProcesso, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM str_processo WHERE cd_processo=?");
			pstmt.setInt(1, cdProcesso);
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM str_processo", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

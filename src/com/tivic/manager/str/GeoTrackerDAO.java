...
public class GeoTrackerDAO{
	...
	public static int insert(GeoTracker objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("str_geo_tracker", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO str_geo_tracker (cd_tracker,"+
			                                  "id_orgao,"+
			                                  "id_equipamento,"+
			                                  "nr_matricula_agente,"+
			                                  "vl_latitude,"+
	...
	public static int update(GeoTracker objeto, int cdTrackerOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE str_geo_tracker SET cd_tracker=?,"+
												      		   "id_orgao=?,"+
												      		   "id_equipamento=?,"+
												      		   "nr_matricula_agente=?,"+
												      		   "vl_latitude=?,"+
			...
			pstmt.setInt(8,objeto.getCdOrgao());
			pstmt.setInt(9,objeto.getCdAgente());
			pstmt.setInt(10, cdTrackerOld!=0 ? cdTrackerOld : objeto.getCdTracker());
			pstmt.executeUpdate();
			return 1;
		}
		catch(SQLException sqlExpt){
	...
	public static int delete(int cdTracker, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM str_geo_tracker WHERE cd_tracker=?");
	...
	public static GeoTracker get(int cdTracker, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM str_geo_tracker WHERE cd_tracker=?");
			pstmt.setInt(1, cdTracker);
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM str_geo_tracker", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

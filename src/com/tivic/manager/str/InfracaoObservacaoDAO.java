...
public class InfracaoObservacaoDAO{
	...
	public static int insert(InfracaoObservacao objeto, Connection connect){
		...
		try {
			...
			keys[1].put("FIELD_NAME", "COD_INFRACAO");
			keys[1].put("IS_KEY_NATIVE", "NO");
			keys[1].put("FIELD_VALUE", new Integer(objeto.getCdInfracao()));
			int code = Conexao.getSequenceCode("STR_INFRACAO_OBSERVACAO", keys, connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO STR_INFRACAO_OBSERVACAO (COD_INFRACAO,"+
			                                  "CD_OBSERVACAO,"+
			                                  "NR_OBSERVACAO,"+
			                                  "NM_OBSERVACAO,"+
			                                  "TXT_OBSERVACAO,"+
	...
	public static int update(InfracaoObservacao objeto, int cdObservacaoOld, int cdInfracaoOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE STR_INFRACAO_OBSERVACAO SET COD_INFRACAO=?,"+
												      		   "CD_OBSERVACAO=?,"+
												      		   "NR_OBSERVACAO=?,"+
												      		   "NM_OBSERVACAO=?,"+
												      		   "TXT_OBSERVACAO=?,"+
												      		   "ST_OBSERVACAO=? WHERE CD_OBSERVACAO=? AND COD_INFRACAO=?");
			pstmt.setInt(1,objeto.getCdInfracao());
	...
	public static int delete(int cdObservacao, int cdInfracao, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM STR_INFRACAO_OBSERVACAO WHERE CD_OBSERVACAO=? AND COD_INFRACAO=?");
	...
	public static InfracaoObservacao get(int cdObservacao, int cdInfracao, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM STR_INFRACAO_OBSERVACAO WHERE CD_OBSERVACAO=? AND COD_INFRACAO=?");
			pstmt.setInt(1, cdObservacao);
			pstmt.setInt(2, cdInfracao);
			rs = pstmt.executeQuery();
			if(rs.next()){
				return new InfracaoObservacao(rs.getInt("COD_INFRACAO"),
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM STR_INFRACAO_OBSERVACAO");
	...
	public static ArrayList<InfracaoObservacao> getList(Connection connect) {
		...
		try {
			...
			while(rsm.next()){
				InfracaoObservacao obj = InfracaoObservacaoDAO.get(rsm.getInt("CD_OBSERVACAO"), rsm.getInt("COD_INFRACAO"), connect);
				list.add(obj);
			}
			return list;
		}
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM STR_INFRACAO_OBSERVACAO", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

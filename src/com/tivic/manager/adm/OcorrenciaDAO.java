...
public class OcorrenciaDAO{
	...
	public static int insert(Ocorrencia objeto, Connection connect){
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO adm_ocorrencia (cd_ocorrencia,"+
			                                  "cd_titulo_credito,"+
			                                  "cd_contrato) VALUES (?, ?, ?)");
			...
			if(objeto.getCdTituloCredito()==0)
				...
				pstmt.setInt(2,objeto.getCdTituloCredito());
	...
	public static int update(Ocorrencia objeto, int cdOcorrenciaOld, Connection connect){
		...
		try {
			...
			if (objetoTemp == null)
				pstmt = connect.prepareStatement("INSERT INTO adm_ocorrencia (cd_ocorrencia,"+
			                                  "cd_titulo_credito,"+
			                                  "cd_contrato) VALUES (?, ?, ?)");
			else
				pstmt = connect.prepareStatement("UPDATE adm_ocorrencia SET cd_ocorrencia=?,"+
												      		   "cd_titulo_credito=?,"+
												      		   "cd_contrato=? WHERE cd_ocorrencia=?");
			pstmt.setInt(1,objeto.getCdOcorrencia());
			if(objeto.getCdTituloCredito()==0)
				pstmt.setNull(2, Types.INTEGER);
			else
	...
	public static int delete(int cdOcorrencia, Connection connect){
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM adm_ocorrencia WHERE cd_ocorrencia=?");
	...
	public static Ocorrencia get(int cdOcorrencia, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM adm_ocorrencia A, grl_ocorrencia B WHERE A.cd_ocorrencia=B.cd_ocorrencia AND A.cd_ocorrencia=?");
			pstmt.setInt(1, cdOcorrencia);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM adm_ocorrencia");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM adm_ocorrencia", criterios, true, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

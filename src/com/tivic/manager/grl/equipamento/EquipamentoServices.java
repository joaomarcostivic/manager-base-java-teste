...
public class EquipamentoServices {
	...
	public static final String[] tiposEquipamento  = {
			...
			"Radar Estático", "Detector", "Zona Azul"};
	...
	public static Equipamento getByIdAndTp(String idEquipamento, Integer tpEquipamento) {
		return getByIdAndTp(idEquipamento,tpEquipamento, null);
	}

	public static Equipamento getByIdAndTp(String idEquipamento,Integer tpEquipamento, Connection connect){
		boolean isConnectionNull = connect==null;
		if (isConnectionNull)
			connect = Conexao.conectar();
		PreparedStatement pstmt;
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM grl_equipamento WHERE id_equipamento=? AND tp_equipamento=?");
			pstmt.setString(1, idEquipamento);
			pstmt.setInt(2, tpEquipamento);

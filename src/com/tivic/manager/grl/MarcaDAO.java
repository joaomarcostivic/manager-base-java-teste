...
public class MarcaDAO{
	...
	public static int insert(Marca objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("grl_marca", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO grl_marca (cd_marca,"+
			                                  "nm_marca,"+
			                                  "id_marca) VALUES (?, ?, ?)");
			pstmt.setInt(1, code);
	...
	public static int update(Marca objeto, int cdMarcaOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE grl_marca SET cd_marca=?,"+
												      		   "nm_marca=?,"+
	...
	public static int delete(int cdMarca, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM grl_marca WHERE cd_marca=?");
	...
	public static Marca get(int cdMarca, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM grl_marca WHERE cd_marca=?");
			pstmt.setInt(1, cdMarca);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM grl_marca");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM grl_marca", criterios, true, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

...
public class TipoOcorrenciaDAO{
	...
	public static int insert(TipoOcorrencia objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("grl_tipo_ocorrencia", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO grl_tipo_ocorrencia (cd_tipo_ocorrencia,"+
			                                  "nm_tipo_ocorrencia,"+
			                                  "id_tipo_ocorrencia,"+
			                                  "lg_email) VALUES (?, ?, ?, ?)");
			pstmt.setInt(1, code);
	...
	public static int update(TipoOcorrencia objeto, int cdTipoOcorrenciaOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE grl_tipo_ocorrencia SET cd_tipo_ocorrencia=?,"+
												      		   "nm_tipo_ocorrencia=?,"+
												      		   "id_tipo_ocorrencia=?,"+
	...
	public static int delete(int cdTipoOcorrencia, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM grl_tipo_ocorrencia WHERE cd_tipo_ocorrencia=?");
	...
	public static TipoOcorrencia get(int cdTipoOcorrencia, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM grl_tipo_ocorrencia WHERE cd_tipo_ocorrencia=?");
			pstmt.setInt(1, cdTipoOcorrencia);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM grl_tipo_ocorrencia");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM grl_tipo_ocorrencia", criterios, true, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

...
public class FuncaoDAO{
	...
	public static int insert(Funcao objeto, Connection connect){
		...
		try {
			int code = Conexao.getSequenceCode("grl_funcao", connect);
			if (code <= 0) {
				if (isConnectionNull)
					Conexao.rollback(connect);
				return -1;
			}
			pstmt = connect.prepareStatement("INSERT INTO grl_funcao (cd_funcao,"+
			                                  "cd_empresa,"+
			                                  "nm_funcao,"+
			                                  "id_funcao) VALUES (?, ?, ?, ?)");
			pstmt.setInt(1, code);
	...
	public static int update(Funcao objeto, Connection connect){
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("UPDATE grl_funcao SET cd_empresa=?,"+
			                                  "nm_funcao=?,"+
			...
			pstmt.setString(2,objeto.getNmFuncao());
			pstmt.setString(3,objeto.getIdFuncao());
	...
	public static int delete(int cdFuncao, Connection connect){
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("DELETE FROM grl_funcao WHERE cd_funcao=?");
	...
	public static Funcao get(int cdFuncao, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM grl_funcao WHERE cd_funcao=?");
			pstmt.setInt(1, cdFuncao);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM grl_funcao");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM grl_funcao", criterios, true, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

...
public class GrupoDAO{
	...
	public static int insert(Grupo objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("sinc_grupo", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO sinc_grupo (cd_grupo,"+
			                                  "nm_grupo,"+
			                                  "st_grupo) VALUES (?, ?, ?)");
			pstmt.setInt(1, code);
			pstmt.setString(2,objeto.getNmGrupo());
			pstmt.setInt(3,objeto.getStGrupo());
			pstmt.executeUpdate();
			return code;
		}
	...
	public static int update(Grupo objeto, int cdGrupoOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE sinc_grupo SET cd_grupo=?,"+
												      		   "nm_grupo=?,"+
	...
	public static int delete(int cdGrupo, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM sinc_grupo WHERE cd_grupo=?");
	...
	public static Grupo get(int cdGrupo, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM sinc_grupo WHERE cd_grupo=?");
			pstmt.setInt(1, cdGrupo);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM sinc_grupo");

...
public class LocalDAO{
	...
	public static int insert(Local objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("sinc_local", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO sinc_local (cd_local,"+
			                                  "id_local,"+
			                                  "tp_local,"+
			                                  "nm_login_database,"+
			                                  "nm_senha_database,"+
			                                  "nm_url_database) VALUES (?, ?, ?, ?, ?, ?)");
			pstmt.setInt(1, code);
			pstmt.setString(2,objeto.getIdLocal());
			pstmt.setInt(3,objeto.getTpLocal());
			pstmt.setString(4,objeto.getNmLoginDatabase());
			pstmt.setString(5,objeto.getNmSenhaDatabase());
			pstmt.setString(6,objeto.getNmUrlDatabase());
			pstmt.executeUpdate();
			return code;
		}
	...
	public static int update(Local objeto, int cdLocalOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE sinc_local SET cd_local=?,"+
												      		   "id_local=?,"+
												      		   "tp_local=?,"+
												      		   "nm_login_database=?,"+
												      		   "nm_senha_database=?,"+
												      		   "nm_url_database=? WHERE cd_local=?");
			...
			pstmt.setString(5,objeto.getNmSenhaDatabase());
			pstmt.setString(6,objeto.getNmUrlDatabase());
			pstmt.setInt(7, cdLocalOld!=0 ? cdLocalOld : objeto.getCdLocal());
			pstmt.executeUpdate();
			return 1;
		}
		catch(SQLException sqlExpt){
	...
	public static int delete(int cdLocal, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM sinc_local WHERE cd_local=?");
	...
	public static Local get(int cdLocal, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM sinc_local WHERE cd_local=?");
			pstmt.setInt(1, cdLocal);
			...
			if(rs.next()){
				return new Local(rs.getInt("cd_local"),
						...
						rs.getInt("tp_local"),
						rs.getString("nm_login_database"),
						rs.getString("nm_senha_database"),
						rs.getString("nm_url_database"));
			}
			else{
				return null;
			}

...
public class GerarDocumentoJulgamentoDefesaDeferida implements IGerarDocumentosLoteJulgamento {
	...
	public GerarDocumentoJulgamentoDefesaDeferida() throws Exception {
	...
	public byte[] gerarDocumentos(LoteImpressao	 loteImpressao, int cdUsuario, CustomConnection connection) throws Exception {
		...
		for (LoteImpressaoAit loteImpressaoAit: loteImpressaoAitList) {
			Report reportDocumentoDefesaDeferida = findDocumentoDefesaDeferida(loteImpressaoAit, reportCriterios);
	...
	private ReportCriterios montarReportCriterios() throws Exception {
		...
		reportCriterios.addParametros("DS_SUBTITULO", "ACOLHIMENTO DE DEFESA DA AUTUAÇÃO");
	...
	private void montarDadosDocumento(List<DadosCartaJulgamento> listDadosCartaJulgamento, ReportCriterios reportCriterios) throws Exception {
		for (DadosCartaJulgamento dadosCartaJulgamento : listDadosCartaJulgamento) {
			alteraTextoDefesaDeferida(dadosCartaJulgamento);
			reportCriterios.addParametros("MOB_BLB_MENSAGEM_RESULTADO", dadosCartaJulgamento.getDefesaPreviaDeferida());
		}
	}
	
	private void alteraTextoDefesaDeferida(DadosCartaJulgamento dadosCartaJulgamento) throws Exception {
			String textoResultado = ParametroServices.getValorOfParametroAsString("MOB_BLB_DEFESA_PREVIA_DEFERIDA", "");
			...
			dadosCartaJulgamento.setDefesaPreviaDeferida(textoResultado);
	}
	...
	private Report findDocumentoDefesaDeferida(LoteImpressaoAit loteImpressaoAit, ReportCriterios reportCriterios) throws Exception {
		SearchCriterios searchCriterios = montarCriterios(loteImpressaoAit.getCdAit());
		List<DadosCartaJulgamento> listDadosJulgamento = searchDefesaDeferida(searchCriterios).getList(DadosCartaJulgamento.class);
	...
	private Search<DadosCartaJulgamento> searchDefesaDeferida(SearchCriterios searchCriterios) throws Exception {
		Search<DadosCartaJulgamento> search = new SearchBuilder<DadosCartaJulgamento>("MOB_AIT A")
				...
				.addJoinTable(" LEFT OUTER JOIN mob_ait_movimento	   K  ON (A.cd_ait = K.cd_ait AND K.tp_status = " + TipoStatusEnum.DEFESA_DEFERIDA.getKey() + " )")

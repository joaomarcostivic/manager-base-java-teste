...
public class GerarDocumentoJulgamentoJariDeferida implements IGerarDocumentosLoteJulgamento {
	...
	public GerarDocumentoJulgamentoJariDeferida() throws Exception {
	...
	public byte[] gerarDocumentos(LoteImpressao	 loteImpressao, int cdUsuario, CustomConnection connection) throws Exception {
		...
		for (LoteImpressaoAit loteImpressaoAit: loteImpressaoAitList) {
			Report reportDocumentoDefesaDeferida = findDocumentoJariDeferida(loteImpressaoAit, reportCriterios);
	...
	private ReportCriterios montarReportCriterios() throws Exception {
		...
		reportCriterios.addParametros("DS_SUBTITULO", "DEFERIMENTO DO RECURSO - 1ª INSTÂNCIA - JARI");
	...
	private void montarDadosDocumento(List<DadosCartaJulgamento> listDadosCartaJulgamento, ReportCriterios reportCriterios) throws Exception {
		for (DadosCartaJulgamento dadosCartaJulgamento : listDadosCartaJulgamento) {
			alteraTextoJariDeferida(dadosCartaJulgamento);
			reportCriterios.addParametros("MOB_BLB_MENSAGEM_RESULTADO", dadosCartaJulgamento.getJariDeferida());
		}
	}
	
	private void alteraTextoJariDeferida(DadosCartaJulgamento dadosCartaJulgamento) throws Exception {
			String textoResultado = ParametroServices.getValorOfParametroAsString("MOB_BLB_JARI_COM_PROVIMENTO", "");
			...
			dadosCartaJulgamento.setJariDeferida(textoResultado);
	}
	...
	private Report findDocumentoJariDeferida(LoteImpressaoAit loteImpressaoAit, ReportCriterios reportCriterios) throws Exception {
		SearchCriterios searchCriterios = montarCriterios(loteImpressaoAit.getCdAit());
		List<DadosCartaJulgamento> listDadosJulgamento = searchJariDeferida(searchCriterios).getList(DadosCartaJulgamento.class);
	...
	private Search<DadosCartaJulgamento> searchJariDeferida(SearchCriterios searchCriterios) throws Exception {
		Search<DadosCartaJulgamento> search = new SearchBuilder<DadosCartaJulgamento>("MOB_AIT A")
				...
				.addJoinTable(" LEFT OUTER JOIN mob_ait_movimento	   K  ON (A.cd_ait = K.cd_ait AND K.tp_status = " + TipoStatusEnum.JARI_COM_PROVIMENTO.getKey() + " )")

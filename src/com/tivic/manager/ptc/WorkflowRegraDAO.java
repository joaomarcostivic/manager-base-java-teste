...
public class WorkflowRegraDAO{
	...
	public static int insert(WorkflowRegra objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("ptc_workflow_regra", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO ptc_workflow_regra (cd_regra,"+
			                                  "nm_regra,"+
			                                  "st_regra,"+
			                                  "dt_cadastro,"+
			                                  "id_regra,"+
	...
	public static int update(WorkflowRegra objeto, int cdRegraOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE ptc_workflow_regra SET cd_regra=?,"+
												      		   "nm_regra=?,"+
												      		   "st_regra=?,"+
												      		   "dt_cadastro=?,"+
												      		   "id_regra=?,"+
	...
	public static int delete(int cdRegra, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM ptc_workflow_regra WHERE cd_regra=?");
	...
	public static WorkflowRegra get(int cdRegra, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM ptc_workflow_regra WHERE cd_regra=?");
			pstmt.setInt(1, cdRegra);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM ptc_workflow_regra");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM ptc_workflow_regra", criterios, true, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

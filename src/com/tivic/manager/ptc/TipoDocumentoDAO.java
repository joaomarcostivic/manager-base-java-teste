...
public class TipoDocumentoDAO{
	...
	public static int insert(TipoDocumento objeto, Connection connect){
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO ptc_tipo_documento (cd_tipo_documento) VALUES (?)");
	...
	public static int update(TipoDocumento objeto, int cdTipoDocumentoOld, Connection connect){
		...
		try {
			...
			if (objetoTemp == null) 
				pstmt = connect.prepareStatement("INSERT INTO ptc_tipo_documento (cd_tipo_documento) VALUES (?)");
			else
				pstmt = connect.prepareStatement("UPDATE ptc_tipo_documento SET cd_tipo_documento=? WHERE cd_tipo_documento=?");
			pstmt.setInt(1,objeto.getCdTipoDocumento());
			if (objetoTemp != null) {
	...
	public static int delete(int cdTipoDocumento, Connection connect){
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM ptc_tipo_documento WHERE cd_tipo_documento=?");
	...
	public static TipoDocumento get(int cdTipoDocumento, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT B.* FROM ptc_tipo_documento A, gpn_tipo_documento B WHERE A.cd_tipo_documento=B.cd_tipo_documento AND A.cd_tipo_documento=?");
			pstmt.setInt(1, cdTipoDocumento);
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM ptc_tipo_documento", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

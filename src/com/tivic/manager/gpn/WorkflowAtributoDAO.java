...
public class WorkflowAtributoDAO{
	...
	public static int insert(WorkflowAtributo objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("gpn_workflow_atributo", keys, connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO gpn_workflow_atributo (cd_atributo,"+
			                                  "cd_entidade,"+
			                                  "nm_atributo,"+
			                                  "id_atributo,"+
			                                  "tp_atributo,"+
			...
			pstmt.setString(4,objeto.getIdAtributo());
			pstmt.setInt(5,objeto.getTpAtributo());
			pstmt.setString(6,objeto.getNmClasspath());
			pstmt.executeUpdate();
			return code;
		}
	...
	public static int update(WorkflowAtributo objeto, int cdAtributoOld, int cdEntidadeOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE gpn_workflow_atributo SET cd_atributo=?,"+
												      		   "cd_entidade=?,"+
												      		   "nm_atributo=?,"+
												      		   "id_atributo=?,"+
												      		   "tp_atributo=?,"+
												      		   "nm_classpath=? WHERE cd_atributo=? AND cd_entidade=?");
			...
			pstmt.setInt(5,objeto.getTpAtributo());
			pstmt.setString(6,objeto.getNmClasspath());
			pstmt.setInt(7, cdAtributoOld!=0 ? cdAtributoOld : objeto.getCdAtributo());
			pstmt.setInt(8, cdEntidadeOld!=0 ? cdEntidadeOld : objeto.getCdEntidade());
			pstmt.executeUpdate();
			return 1;
		}
		catch(SQLException sqlExpt){
	...
	public static int delete(int cdAtributo, int cdEntidade, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM gpn_workflow_atributo WHERE cd_atributo=? AND cd_entidade=?");
	...
	public static WorkflowAtributo get(int cdAtributo, int cdEntidade, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM gpn_workflow_atributo WHERE cd_atributo=? AND cd_entidade=?");
			pstmt.setInt(1, cdAtributo);
			pstmt.setInt(2, cdEntidade);
			...
			if(rs.next()){
				return new WorkflowAtributo(rs.getInt("cd_atributo"),
						...
						rs.getInt("tp_atributo"),
						rs.getString("nm_classpath"));
			}
			else{
				return null;
			}
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM gpn_workflow_atributo");

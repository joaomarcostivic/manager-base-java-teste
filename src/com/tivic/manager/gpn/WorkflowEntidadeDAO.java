...
public class WorkflowEntidadeDAO{
	...
	public static int insert(WorkflowEntidade objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("gpn_workflow_entidade", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO gpn_workflow_entidade (cd_entidade,"+
			                                  "nm_entidade,"+
			                                  "id_entidade,"+
			                                  "nm_classpath) VALUES (?, ?, ?, ?)");
			pstmt.setInt(1, code);
			pstmt.setString(2,objeto.getNmEntidade());
			pstmt.setString(3,objeto.getIdEntidade());
			pstmt.setString(4,objeto.getNmClasspath());
			pstmt.executeUpdate();
			return code;
		}
	...
	public static int update(WorkflowEntidade objeto, int cdEntidadeOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE gpn_workflow_entidade SET cd_entidade=?,"+
												      		   "nm_entidade=?,"+
												      		   "id_entidade=?,"+
												      		   "nm_classpath=? WHERE cd_entidade=?");
	...
	public static int delete(int cdEntidade, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM gpn_workflow_entidade WHERE cd_entidade=?");
	...
	public static WorkflowEntidade get(int cdEntidade, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM gpn_workflow_entidade WHERE cd_entidade=?");
			pstmt.setInt(1, cdEntidade);
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM gpn_workflow_entidade", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

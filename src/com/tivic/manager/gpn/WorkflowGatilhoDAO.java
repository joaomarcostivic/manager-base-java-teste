...
public class WorkflowGatilhoDAO{
	...
	public static int insert(WorkflowGatilho objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("gpn_workflow_gatilho", keys, connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO gpn_workflow_gatilho (cd_gatilho,"+
			                                  "cd_regra,"+
			                                  "tp_gatilho,"+
			                                  "vl_inicial,"+
			                                  "vl_final,"+
			...
			if(objeto.getCdTipoAndamento()==0)
				...
				pstmt.setInt(10,objeto.getCdTipoAndamento());
			if(objeto.getCdPessoa()==0)
				pstmt.setNull(11, Types.INTEGER);
			else
				pstmt.setInt(11,objeto.getCdPessoa());
			pstmt.setInt(12,objeto.getTpAgendaItem());
			pstmt.executeUpdate();
			return code;
		}
	...
	public static int update(WorkflowGatilho objeto, int cdGatilhoOld, int cdRegraOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE gpn_workflow_gatilho SET cd_gatilho=?,"+
												      		   "cd_regra=?,"+
												      		   "tp_gatilho=?,"+
												      		   "vl_inicial=?,"+
												      		   "vl_final=?,"+
												      		   ...
												      		   "cd_tipo_andamento=?,"+
												      		   "cd_pessoa=?,"+
												      		   "tp_agenda_item=? WHERE cd_gatilho=? AND cd_regra=?");
			...
			if(objeto.getCdPessoa()==0)
				...
				pstmt.setInt(11,objeto.getCdPessoa());
			pstmt.setInt(12,objeto.getTpAgendaItem());
			pstmt.setInt(13, cdGatilhoOld!=0 ? cdGatilhoOld : objeto.getCdGatilho());
	...
	public static int delete(int cdGatilho, int cdRegra, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM gpn_workflow_gatilho WHERE cd_gatilho=? AND cd_regra=?");
	...
	public static WorkflowGatilho get(int cdGatilho, int cdRegra, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM gpn_workflow_gatilho WHERE cd_gatilho=? AND cd_regra=?");
			pstmt.setInt(1, cdGatilho);
			pstmt.setInt(2, cdRegra);
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM gpn_workflow_gatilho", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

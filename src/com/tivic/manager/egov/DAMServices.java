...
public class DAMServices {
	...
	public static Result getDAM(String nrInscricaoMunicipal)	{
		...
		try	{
			...
			//	return new Result(-1, "A TLL do ano de "+(new GregorianCalendar().get(java.util.Calendar.YEAR))+" ainda nao foi gerado! Aguarde...");
			/*
			 * DADOS DO CONTRIBUINTE - Validando numero da inscricao
			 */
			...
			if(nrInscricaoMunicipal==null || nrInscricaoMunicipal.equals(""))
				return new Result(-1, "Informe a InscriÃ§Ã£o Municipal!");
			//
			...
			if(rsm.next())	{
				...
				return new Result(-1, "NÂº de InscriÃ§Ã£o InvÃ¡lida!");
			...
			/*
			 *  Recuperando informacoes dos parametros
			 */
			...
			while(rs.next())	{
				for(int i=1; i<=6; i++)	{
					...
					// Se houver os dois pontos no campo do valor, nao e uma parcela
					// Se a data de vencimento nao existe dao loop, se a data de pagamento estiver informada ja foi PAGO
					...
					// So gera uma parcela do tributo
					...
					float vlTributo  = rs.getFloat("vlParcela"+i) / 100; // Valor ja vem sem casa decimais
					vlTotal         += vlTributo;
					if(vlTributo > 0) {
						String nmTributo = rs.getInt("cdTributo")==2101 ? "TLL - TAXA DE LICENCA PARA FUNCIONAMENTO" : "ISSP - ISS SOCIEDAD.PROFISSIONAIS";
						...
						// Calculando juros da parcela em questao
						if(!dtVencimento.after(new GregorianCalendar()))	{
							...
							dsMensagem = "Recalculada em "+com.tivic.manager.util.Util.formatDateTime(new GregorianCalendar(), "dd/MM/yyyy")+" para pagamento ate o dia "+
			...
			while(rs.next())	{
				for(int i=1; i<=4; i++)	{
					...
					String venc  = rs.getString("dtVencimento"+i); 
					String pagto = rs.getString("dtPagamento"+i);
					// Se a data de vencimento ou valor nao existir d o loop, se a data de pagamento estiver informada ja foi PAGO
			...
			if(rsmTributos.size()==0)
				return new Result(-1, "NÃ£o existe tributo do ano base "+nrAnoBase+" em aberto!");
			// Multa
			if(vlMulta > 0) {
				HashMap<String,Object> register = new HashMap<String,Object>();
				register.put("CD_TRIBUTO", Util.fillNum(rsmTributos.size()+1, 2));
			...
			if(rs.next())	{
				...
				return new Result(-1, "NÃ£o existe documento de arrecadaÃ§Ã£o para essa inscriÃ§Ã£o. Comunique a SEFIN!");
			// System.out.println("\nBuscou numero DAM. "+((new GregorianCalendar().getTimeInMillis() - init.getTimeInMillis()) / 1000)+"s");
			/*
			 *  CONTA A RECEBER - Registrando emissao
			 */
			int cdEmpresa = 0;
			...
			/*
			 *  ISSFFC01DAM - Registrando emissao
			 */
			// Excluindo
			...
			result.addObject("nmCidade", "VITÃRIA DA CONQUISTA");
	...
	public static Result getCodigoBarrasConvenio(String idContaReceber, float vlContaReceber, GregorianCalendar dtVencimento)	{
		Result result = new Result(1);
		...
		try	{
			...
			idContaReceber = com.tivic.manager.util.Util.fill(idContaReceber, 17, '0', 'E'); 
			// Formando codigo de barras
			String nrCodigoBarras = "8"+ // 8 = Impostos
									"1"+ //
									"7"+ // Valor/Referencia: 7 = Referencia
									com.tivic.manager.util.Util.fillNum(Math.round(vlContaReceber * 100), 11)+ // Valor da parcela com 11 posicoes
									"4785"+ // Cadigo FEBRABAN
									com.tivic.manager.util.Util.formatDate(dtVencimento, "yyyyMMdd")+
									idContaReceber;
//			System.out.println(nrCodigoBarras);
			
			// Acrescentando digito verificador
			...
			result.addObject("nrCodigoBarras", nrCodigoBarras);
			result.addObject("bloco1", bloco1);
			result.addObject("bloco2", bloco2);
			result.addObject("bloco3", bloco3);
			result.addObject("bloco4", bloco4);
	...
	public static String[] getCodigoBarras(String nrDAM, float vlParcela, String venc, int nrParcela)	{
		...
		try	{
			...
			String nrCodigoBarras = "8"+ // 8 = Impostos
									"1"+ //
									"7"+ // Valor/Referencia: 7 = Referencia
									com.tivic.manager.util.Util.fillNum(Math.round(vlParcela * 100), 11)+ // Valor da parcela com 11 posicoes
									"4785"+ // Codigo FEBRABAN
									com.tivic.manager.util.Util.formatDate(dtVencimento, "yyyyMMdd")+
									nrDAM;
			// Acrescentando digito verificador
			nrCodigoBarras = nrCodigoBarras.subSequence(0,3)+
	...
	/**
	 * Metodo para gerar numero de DAM atraves de numeracao do banco da E&L
	 * @param Informacoes necessarias para busca e insercao de pessoa e taxa no banco
	 * @return Numero do DAM (Result)
	 */
	public static Result getCodigoBarrasEL(String ano, GregorianCalendar dtVencimento, float valor, float fator,
			String nome, String tipoPessoa, String documento, String nmFantasia,
			int cdPessoa, int cdDocumento, int cdDocumentoConta, int cdContaReceber, int cdUsuario) {
		return getCodigoBarrasEL(ano, dtVencimento, valor, fator, nome, tipoPessoa, documento, nmFantasia, 
					cdPessoa, cdDocumento, cdDocumentoConta, cdContaReceber, cdUsuario, null);
	}
	
	public static Result getCodigoBarrasEL(String ano, GregorianCalendar dtVencimento, float valor, float fator,
									String nome, String tipoPessoa, String documento, String nmFantasia, 
									int cdPessoa, int cdDocumento, int cdDocumentoConta, int cdContaReceber, int cdUsuario, Connection connection) {
		...
		DocumentoConta dc = DocumentoContaDAO.get(cdDocumento, cdDocumentoConta, connection);
		
		// Caso o codigo de barras ja tenha sido gerado anteriormente, retorna ele.
//		if(dc.getNrDocumento()!=null && dc.getNrDocumento().length()>0 && dc.getStDocumentoConta()==DocumentoConta.ST_ATIVO){
//			String nrCodigoBarras = dc.getNrDocumento();
//			
//			String bloco1 = nrCodigoBarras.substring(0, 11);
//			String bloco2 = nrCodigoBarras.substring(11, 22);
//			String bloco3 = nrCodigoBarras.substring(22, 33);
//			String bloco4 = nrCodigoBarras.substring(33, 44);
//			
//			Result r = new Result(1);
//			r.addObject("nrCodigoBarras", nrCodigoBarras);
//			r.addObject("bloco1", bloco1);
//			r.addObject("bloco2", bloco2);
//			r.addObject("bloco3", bloco3);
//			r.addObject("bloco4", bloco4);
//			
//			return r;
//		}
		
		// Buscar codigo da pessoa no banco PROCON
		pstmt = connection.prepareStatement("SELECT * FROM grl_pessoa_externa WHERE cd_pessoa = ?");
		pstmt.setInt(1, cdPessoa);
		...
		if(rs.next()){
			...
			// Buscar codigo geral da empresa no banco EL
			codigoG = DAMUtils.getPessoa(nome, tipoPessoa, documento, nmFantasia);
			
			if (codigoG != null) {
				// Salvar no banco PROCON
				pstmt = connection.prepareStatement("INSERT INTO grl_pessoa_externa(cd_pessoa, cd_pessoa_externa) VALUES (?, ?)");
				pstmt.setInt(1, cdPessoa);
				pstmt.setString(2, codigoG);
			}
		}
		
		if (codigoG != null) {
			// Incluir a taxa no banco da E&L
			codigoTxc = DAMUtils.insertTaxa(ano, codigoG, dtVencimento, valor, fator); //codigoTxc = pLivre = Codigo do Movimento
			
			if (codigoTxc != null) {
				// Gerar codigo de barras
				String nrDAM = ano + codigoTxc + "09910";
				Result r = getCodigoBarrasConvenio(nrDAM, valor, dtVencimento);
				
				System.out.println("BLOCO 1 = " + r.getObjects().get("bloco1"));
				System.out.println("BLOCO 2 = " + r.getObjects().get("bloco2"));
				System.out.println("BLOCO 3 = " + r.getObjects().get("bloco3"));
				System.out.println("BLOCO 4 = " + r.getObjects().get("bloco4"));
				
				// Salvar no banco PROCON
				dc.setCodMovimento(codigoTxc);
				dc.setNrDocumento((String)r.getObjects().get("nrCodigoBarras"));
				int retorno = DocumentoContaDAO.update(dc, connection);
				
				//GERAR OCORRENCIA
				ContaReceber cr = ContaReceberDAO.get(cdContaReceber, connection);
				Pessoa sacado = PessoaDAO.get(cr.getCdPessoa());
				
				int cdOcorrencia = ParametroServices.getValorOfParametroAsInteger("CD_TIPO_OCORRENCIA_DAM", 0, 0, connection);
				if(cdOcorrencia<=0) {
					return new Result(-6, "Ocorrência padrão de DAM gerado não encontrada.");
				}
				DocumentoOcorrencia documentoOcorrencia = new DocumentoOcorrencia(dc.getCdDocumento(), 
						cdOcorrencia, 
						0/*cdOcorrencia*/, 
						cdUsuario, 
						new GregorianCalendar(), 
						"DAM enviado para empresa "+sacado.getNmPessoa()+ " no valor de R$ "+cr.getVlConta() + " com o vencimento para "
								+ Util.formatDateTime(cr.getDtVencimento(), "dd/MM/yyyy")+".", 
						DocumentoOcorrenciaServices.TP_VISIBILIDADE_PUBLICO, 0);
				
				if(DocumentoOcorrenciaServices.save(documentoOcorrencia, connection).getCode() <= 0) {
				...
				return new Result(-2, "Erro ao incluir taxa ou taxa já existente.");
			}
		}
		...
		return null;
			...
			e.printStackTrace(System.out);
			System.err.println("Erro! DAMServices.getCodigoBarrasEL: " +  e);
			if (isConnectionNull)
				Conexao.rollback(connection);
			return new Result(-1, "Erro ao gerar código");
		}
		finally{
			if (isConnectionNull)
	...
	/**
	 * Metodo para verificar no banco da E&L se novas contas foram pagas
	 * @return Contas novas pagas (ResultSetMap)
	 */
	public static ResultSetMap verificarTaxasEL() {
		return verificarTaxasEL(null);
	}

	public static ResultSetMap verificarTaxasEL(Connection connect) {	
		boolean isConnectionNull = connect == null;
		try {
			
			LogUtils.debug("Iniciando verificacao de Taxas E&L..." + Util.formatDate(new GregorianCalendar(), "dd/MM/yyyy HH:mm:ss"));
			...
			// Buscar contas com situacao em aberto ou renegociada
			pstmt = connect.prepareStatement("select A.cd_documento, A.cd_documento_conta, A.cod_movimento, A.dt_pagamento as dt_recebimento, C.cd_pessoa_externa, B.dt_emissao, B.cd_conta_receber" +
											...
											" where B.st_conta = ? or B.st_conta = ?");
			pstmt.setInt(1, ContaReceberServices.ST_EM_ABERTO);
			pstmt.setInt(2, ContaReceberServices.ST_NEGOCIADA);

			ResultSetMap rsmContas = new ResultSetMap(pstmt.executeQuery());
			ResultSetMap rsmPagas = new ResultSetMap();

			String[] taxa = new String[2];
			
			while (rsmContas.next()) {
				DocumentoConta dc = DocumentoContaDAO.get(rsmContas.getInt("cd_documento"), rsmContas.getInt("cd_documento_conta"), connect);
				ContaReceber cr = ContaReceberDAO.get(rsmContas.getInt("cd_conta_receber"));
				
				if (rsmContas.getString("cod_movimento")!="" && rsmContas.getString("cod_movimento")!=null) {
					taxa = DAMUtils.getTaxa(Integer.toString(rsmContas.getGregorianCalendar("dt_emissao").get(Calendar.YEAR)), rsmContas.getString("cod_movimento"));
				
					if (taxa != null) {
						String dtPagamento = taxa[0];
						String vlPago = taxa[1];
						
						double vl = new Double(vlPago.replaceAll(",", "")); 
						GregorianCalendar dt = Util.convStringToCalendar(dtPagamento);
						
						// Salva valor e data de pagamento no banco PROCON
						dc.setVlPagamento(vl);
						dc.setDtPagamento(dt);
						int retornoDocumentoConta = DocumentoContaDAO.update(dc, connect);
						
						// Alterar a situacao da conta para recebida
						cr.setStConta(ContaReceberServices.ST_RECEBIDA);
						int retornoConta = ContaReceberDAO.update(cr, connect);
						
						if(retornoDocumentoConta<=0 && retornoConta>0) {
							connect.rollback();
							return null;
						}
						else {
							HashMap<String, Object> register = new HashMap<String, Object>();
							register.put("CD_DOCUMENTO", rsmContas.getString("cd_documento"));
							register.put("CD_CONTA_RECEBER", rsmContas.getString("cd_conta_receber"));
							
							rsmPagas.addRegister(register);
							connect.commit();
						}
					}
				}
			...
			LogUtils.debug("Verificacao Taxas E&L finalizada.");
			
			return rsmPagas;
		}
		catch(SQLException sqlExpt) {
			sqlExpt.printStackTrace(System.out);
			System.err.println("Erro! DAMServices.verificarTaxasEL: " + sqlExpt);
			return null;
		}
		catch(Exception e) {
			e.printStackTrace(System.out);
			System.err.println("Erro! DAMServices.verificarTaxasEL: " + e);
			return null;
		}
		finally {
			if (isConnectionNull)

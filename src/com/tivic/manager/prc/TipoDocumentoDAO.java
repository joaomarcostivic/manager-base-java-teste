...
public class TipoDocumentoDAO{
	...
	public static int insert(TipoDocumento objeto, Connection connect){
		...
		try {
			...
			int code = com.tivic.manager.gpn.TipoDocumentoDAO.insert(objeto, connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO prc_tipo_documento (cd_tipo_documento,"+
	...
	public static int update(TipoDocumento objeto, int cdTipoDocumentoOld, Connection connect){
		...
		try {
			...
			TipoDocumento objetoTemp = get(objeto.getCdTipoDocumento(), connect);
			if (objetoTemp == null) 
				pstmt = connect.prepareStatement("INSERT INTO prc_tipo_documento (cd_tipo_documento,"+
			                                  "cd_tipo_andamento) VALUES (?, ?)");
			else
				pstmt = connect.prepareStatement("UPDATE prc_tipo_documento SET cd_tipo_documento=?,"+
												      		   "cd_tipo_andamento=? WHERE cd_tipo_documento=?");
			pstmt.setInt(1,objeto.getCdTipoDocumento());
			if(objeto.getCdTipoAndamento()==0)
				pstmt.setNull(2, Types.INTEGER);
			else
				pstmt.setInt(2,objeto.getCdTipoAndamento());
			if (objetoTemp != null) {
				pstmt.setInt(3, cdTipoDocumentoOld!=0 ? cdTipoDocumentoOld : objeto.getCdTipoDocumento());
			}
			pstmt.executeUpdate();
			if (com.tivic.manager.gpn.TipoDocumentoDAO.update(objeto, connect)<=0) {
				if (isConnectionNull)
					Conexao.rollback(connect);
				return -1;
			}
	...
	public static int delete(int cdTipoDocumento, Connection connect){
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM prc_tipo_documento WHERE cd_tipo_documento=?");
			pstmt.setInt(1, cdTipoDocumento);
			pstmt.executeUpdate();
			if (com.tivic.manager.gpn.TipoDocumentoDAO.delete(cdTipoDocumento, connect)<=0) {
				if (isConnectionNull)
					Conexao.rollback(connect);
				return -1;
			}
	...
	public static TipoDocumento get(int cdTipoDocumento, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM prc_tipo_documento A, gpn_tipo_documento B WHERE A.cd_tipo_documento=B.cd_tipo_documento AND A.cd_tipo_documento=?");
			pstmt.setInt(1, cdTipoDocumento);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM prc_tipo_documento");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM prc_tipo_documento", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

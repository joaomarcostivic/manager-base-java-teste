...
public class TipoSituacaoDAO{
	...
	public static int insert(TipoSituacao objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("prc_tipo_situacao", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO prc_tipo_situacao (cd_tipo_situacao,"+
			                                  "nm_tipo_situacao,"+
			                                  "id_tipo_situacao,"+
			                                  "nr_ordem,"+
			                                  "lg_retrocede) VALUES (?, ?, ?, ?, ?)");
			...
			pstmt.setInt(5,objeto.getLgRetrocede());
			pstmt.executeUpdate();
			return code;
		}
	...
	public static int update(TipoSituacao objeto, int cdTipoSituacaoOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE prc_tipo_situacao SET cd_tipo_situacao=?,"+
												      		   "nm_tipo_situacao=?,"+
												      		   "id_tipo_situacao=?,"+
												      		   "nr_ordem=?,"+
												      		   "lg_retrocede=? WHERE cd_tipo_situacao=?");
			pstmt.setInt(1,objeto.getCdTipoSituacao());
			pstmt.setString(2,objeto.getNmTipoSituacao());
			pstmt.setString(3,objeto.getIdTipoSituacao());
			pstmt.setInt(4,objeto.getNrOrdem());
			pstmt.setInt(5,objeto.getLgRetrocede());
			pstmt.setInt(6, cdTipoSituacaoOld!=0 ? cdTipoSituacaoOld : objeto.getCdTipoSituacao());
			pstmt.executeUpdate();
			return 1;
		}
		catch(SQLException sqlExpt){
	...
	public static int delete(int cdTipoSituacao, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM prc_tipo_situacao WHERE cd_tipo_situacao=?");
	...
	public static TipoSituacao get(int cdTipoSituacao, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM prc_tipo_situacao WHERE cd_tipo_situacao=?");
			pstmt.setInt(1, cdTipoSituacao);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM prc_tipo_situacao");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM prc_tipo_situacao", criterios, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

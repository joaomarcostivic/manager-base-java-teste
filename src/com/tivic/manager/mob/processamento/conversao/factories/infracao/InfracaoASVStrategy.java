...
public class InfracaoASVStrategy implements InfracaoStrategy {
	
	private IInfracaoService infracaoService;
	
	public InfracaoASVStrategy(IInfracaoService infracaoService) {
		this.infracaoService = infracaoService;
	}
	
	public Infracao getInfracao(CustomConnection customConnection) throws Exception {
		return infracaoService.getByCodDetran(new ParametroFacade().getVlParametroAsInt("NR_CODIGO_AVANCO_SINAL_VERMELHO", customConnection), customConnection);
	}
}

...
public class ListAitsCandidatosNipViaUnicaService implements IListAitsCandidatasNotificacao {
	private CustomConnection customConnection;
	private List<Ait> aitList;
	private SearchCriterios searchCriterios;

	public ListAitsCandidatosNipViaUnicaService(SearchCriterios searchCriterios, CustomConnection customConnection)
			throws Exception {
	...
	private Search<Ait> searchNip(SearchCriterios searchCriteriosNip) throws Exception {
		Search<Ait> search = new SearchBuilder<Ait>("mob_ait A").customConnection(customConnection)
				...
				.additionalCriterias(" NOT EXISTS " + " (" + "		SELECT B.tp_competencia FROM mob_infracao B "
						+ " 	WHERE " + "			( " + "         	B.tp_competencia = "
						+ TipoCompetenciaEnum.ESTADUAL.getKey() + " 		)" + " 	AND A.cd_infracao = B.cd_infracao "
						+ " )" + " ")
				.orderBy(" B.dt_fim_vigencia desc").build();
		return search;
	}

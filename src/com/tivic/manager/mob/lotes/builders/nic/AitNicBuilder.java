package com.tivic.manager.mob.lotes.builders.nic;
...
import com.tivic.manager.mob.lotes.exception.EnderecoOrgaoException;
...
public class AitNicBuilder {
	private static final int multiplicadorMulta = 2;
	private Ait ait;
	...
	private void construirAit(Ait ait, int cdUsuario, CustomConnection customConnection) throws Exception {
		...
		this.ait.setDsLocalInfracao(getLocalOrgao(customConnection));
		this.ait.setDsPontoReferencia(getPontoReferenciaOrgao(customConnection));
		...
		setVlMulta(ait, customConnection);
	...
	private void setVlMulta(Ait ait, CustomConnection customConnection) throws Exception {
		Infracao infracao = this.infracaoRepository.get(ait.getCdInfracao(), customConnection);
	...
	private Talonario buscarTalaoDisponivel(CustomConnection customConnection) throws Exception {
		...
		return talonarioList.stream().filter(t -> t.getNrUltimoAit() <= t.getNrFinal()).findFirst().orElseThrow(() -> new ValidacaoException("Nenhum talão disponível para a geração da NIC."));
	}

	private String getLocalOrgao(CustomConnection customConnection) throws Exception {
	    String nmLogradouroOrgao = parametroRepository.getValorOfParametroAsString("NM_LOGRADOURO", customConnection);
	    String nrEnderecoOrgao = parametroRepository.getValorOfParametroAsString("NR_ENDERECO", customConnection);
	    String nmBairroOrgao = parametroRepository.getValorOfParametroAsString("NM_BAIRRO", customConnection);
	...
	private String getPontoReferenciaOrgao(CustomConnection customConnection) throws Exception {
	    String nmPontoReferenciaOrgao = parametroRepository.getValorOfParametroAsString("NM_PONTO_REFERENCIA", customConnection);

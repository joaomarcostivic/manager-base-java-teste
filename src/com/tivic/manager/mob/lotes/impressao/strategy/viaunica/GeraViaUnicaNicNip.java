...
import com.tivic.manager.mob.lotes.impressao.strategy.viaunica.validations.INicNipViaUnicaValidator;
import com.tivic.manager.mob.lotes.impressao.strategy.viaunica.validations.NicNipViaUnicaValidator;
...
public class GeraViaUnicaNicNip extends TipoImpressaoNotificacaoNPHandler {
	private ILoteImpressaoService loteImpressaoService;
	private IAitMovimentoService aitMovimentoServices;
	private INicNipViaUnicaValidator nicNipViaUnicaValidator;
	
	public GeraViaUnicaNicNip() throws Exception {
		...
		this.nicNipViaUnicaValidator = new NicNipViaUnicaValidator(); 
	}
	...
	public NipImpressaoDTO gerar(TipoImpressaoNotificacao tipoImpressaoNotificacao, CustomConnection customConnection) throws Exception {
		...
		if (nicNipViaUnicaValidator.validate(tipoImpressaoNotificacao, customConnection)) {
			List<Ait> aitList = new ListAitsCandidatosNipFactory()
					.getEstrategiaListAits(TipoLoteImpressaoEnum.LOTE_NIC_NIP_VIA_UNICA.getKey(), searchNotificacaoViaUnica(tipoImpressaoNotificacao.getCdAit()), customConnection)
					.build();
			finalizarPrazoDefesa(aitList, tipoImpressaoNotificacao.getCdUsuario());
			new GeraMovimentoNotificacaoFactory().strategy(TipoLoteImpressaoEnum.LOTE_NIC_NIP_VIA_UNICA.getKey())
			...
			LoteImpressao loteNotificacaoNipViaUnica = this.loteImpressaoService.gerarLoteNotificacaoAitViaUnica(
					aitList, tipoImpressaoNotificacao.getCdUsuario(), TipoLoteImpressaoEnum.LOTE_NIC_NIP.getKey());

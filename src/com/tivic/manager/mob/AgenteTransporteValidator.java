...
public class AgenteTransporteValidator implements Validator<Talonario> {
	...
	public Optional<String> validateAgente() {
		...
		if(talonario.getCdAgente() <= 0) 
			return Optional.empty();
			
		Agente agente = AgenteDAO.get(talonario.getCdAgente());
		
		if (agente.getTpAgente() == AgenteServices.TP_TRANSPORTE && agente.getStAgente() == AgenteServices.ST_ATIVO)
			return Optional.empty();

		return Optional.of("Agente inválido, o agente deve estar ativo e ser agente de transporte.");
	}
	...
	public Optional<String> validateAlteracaoAgente() {
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("SELECT * FROM MOB_AIT_TRANSPORTE WHERE CD_TALAO = ?");

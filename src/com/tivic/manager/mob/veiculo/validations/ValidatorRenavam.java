...
public class ValidatorRenavam implements Validator{
	...
	public void validate(Veiculo veiculo, Connection connect) throws Exception {
		...
		criterios.add(new ItemComparator("NR_RENAVAM", String.valueOf(veiculo.getNrRenavam()), ItemComparator.EQUAL, Types.VARCHAR));
		ResultSetMap rsm = VeiculoDAO.find(criterios, connect);
		if(rsm.next()) {
			throw new BadRequestException("O Renavam " + rsm.getString("nr_renavam") + " já está associado ao veículo de placa " + rsm.getString("nr_placa"));
		}		
	}
}

...
public class AitMovimentoDocumentoCetranValidator implements Validator<AitMovimentoDocumentoDTO> {
	...
	private Optional<String> validateCetran() {
		...
		try {
			...
			pstmt.setInt(2, AitMovimentoServices.RECURSO_CETRAN);
			...
			if(rsm.next())
				return Optional.of("Já existe outro recurso de CETRAN dessa infração");
	...
	private Optional<String> validateJari() {
		...
		int cdFaseIndeferido = FaseServices.getCdFaseByNome("Indeferido", null);
		...
		try {
			...
			pstmt.setInt(1, this._dto.getMovimento().getCdAit());
			pstmt.setInt(2, AitMovimentoServices.JARI_SEM_PROVIMENTO);
			pstmt.setInt(3, AitMovimentoServices.NAO_ENVIAR);
			pstmt.setInt(4, AitMovimentoServices.REGISTRO_CANCELADO);
			...
			return Optional.of("É necessário um processo na JARI Indefirido para lançar um processo no CETRAN");
		} catch(Exception e) {
			return Optional.of(e.getMessage());
		}
	}
	...
	private Optional<String> validateResultado() {
		...
		try {
			...
			if(rsm.next()) 
				...
				return Optional.of("Não existe entrada de recurso de CETRAN pendente para lançamento de resultado.");
			...
			if(_res.next())
				return Optional.of("Já existe resultado de Recurso no CETRAN pra essa infração.");

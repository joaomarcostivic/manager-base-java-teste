...
public class GerarDocumentosImpressaoNip implements IGerarDocumentosLoteImpressao {
	...
	private String path;
	private AitMovimentoRepository atiMovimentoRepository;
	private IAitMovimentoService aitMovimentoService;
	private IParametroService parametroService;

	public GerarDocumentosImpressaoNip() throws Exception {
		...
		atiMovimentoRepository = (AitMovimentoRepository) BeansFactory.get(AitMovimentoRepository.class);
	...
	public byte[] gerarDocumentos(LoteImpressao loteImpressao, int cdUsuario, CustomConnection connection) throws Exception, ValidacaoException {
		...
		for (int i = 0; i < dadosDocumento.getDadosNotificacaoList().size(); i++) {
			...
			this.content = findDocumentoNip(dadosDocumento.getDadosNotificacaoList().get(i), reportCriterios)
					.getReportPdf(new NomeArquivoImpressao()
					.buscar(getMovimentoPenalidade(dadosDocumento.getDadosNotificacaoList().get(i).getCdAit())));
	...
	private void salvarEtiquetas(List<CorreiosEtiqueta> dadosEtiquetaList) throws Exception {
		for (CorreiosEtiqueta correiosEtiqueta: dadosEtiquetaList) {
			correiosEtiqueta.setTpStatus(TipoStatusEnum.NIP_ENVIADA.getKey());
	...
	private int getMovimentoPenalidade(int cdAit) throws Exception, ValidacaoException {
		...
		List<AitMovimento> aitMovimentoList = this.atiMovimentoRepository.find(searchCriterios, new CustomConnection());

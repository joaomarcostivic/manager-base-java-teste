...
import com.tivic.manager.mob.lote.impressao.validator.LoteNotificacoesNipsValidations;
...
public class GerarMovimentoNotificacaoAdvertenciaLote implements IGerarMovimentoNotificacao {
	...
	public GerarMovimentoNotificacaoAdvertenciaLote() throws Exception {
		...
		iAitInconsistenciaService = (IAitInconsistenciaService) BeansFactory.get(IAitInconsistenciaService.class);
	...
	public List<ServicoDetranDTO> gerarMovimentos(List<Ait> listAits, int cdUsuario) throws Exception {
		...
		try {
			...
			for (Ait ait: listAits) {
				try {		
					managerLog.info("Gerando Movimento de Advertência AIT: ", ait.getIdAit());
					...
					managerLog.info("Erro de validação comum para o AIT: " + ait.getCdAit() + " - ",  e.getMessage());
					servicoDetranDTOListError.add(new ServicoDetranDTOBuilder().build(ait, e.getMessage()));
					continue;
				}
				catch (LoteNotificacaoException e) {
					managerLog.info("Erro de validação inconsistencia para o AIT: " + ait.getCdAit() + " - ", e.getMessage());
					...
					iAitInconsistenciaService.salvarInconsistencia(aitInconsistencia, customConnection);
					servicoDetranDTOListError.add(new ServicoDetranDTOBuilder().build(ait, e.getMessage()));
					continue;
				}
			}

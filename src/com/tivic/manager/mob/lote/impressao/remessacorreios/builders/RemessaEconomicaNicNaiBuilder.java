...
public class RemessaEconomicaNicNaiBuilder implements IBuilderRemessa {
	...
	public RemessaEconomicaNicNaiBuilder() throws Exception {
	...
	public void montarDadosRemessa(LoteImpressao loteImpressao, CustomConnection customConnection) throws Exception {
		...
		dadosDocumento.setNmDocumento((tipoAr == TipoArDigitalEnum.AR_DIGITAL.getKey() ? "mob/nic_nai_remessa_economica_ar_digital": "mob/nic_nai_remessa_economica_ar_digital_2D"));
	...
	private void montarDadosDocumento(List<DadosNotificacao> listDadosNotificacao) throws Exception {
		for (DadosNotificacao dadosNotificacao : listDadosNotificacao) {
			AitMovimento aitMovimento = this.aitMovimentoServices.getMovimentoTpStatus(dadosNotificacao.getCdAit(), TipoStatusEnum.NIC_ENVIADA.getKey());
		...
		this.dadosDocumento.getCriteriosDocumentos().addParametros("MOB_INFORMACOES_ADICIONAIS_NAI", ParametroServices.getValorOfParametro("MOB_INFORMACOES_ADICIONAIS_NAI", customConnection.getConnection()));
	}	

...
public class CartaRegistradaNicNipBuilder implements IBuilderRemessa {
	...
	public CartaRegistradaNicNipBuilder() throws Exception {
	...
	public void montarDadosRemessa(LoteImpressao loteImpressao, CustomConnection customConnection) throws Exception {
		...
		dadosDocumento.setNmDocumento((tipoAr == TipoArDigitalEnum.AR_DIGITAL.getKey() ? "mob/nic_nip_carta_registrada_ar_digital": "mob/nic_nip_carta_registrada_ar_digital_2D"));
	...
	private List<CorreiosEtiqueta> getEtiquetas(int cdLoteNotificacao, List<DadosNotificacao> dadosNotificacaoList) throws Exception {
		...
		for (int index = 0; dadosNotificacaoList.size() > index; index++) {
			new DadosNotificacaoBuilder(dadosNotificacaoList.get(index), correiosEtiquetasList.get(index)).build();
		}

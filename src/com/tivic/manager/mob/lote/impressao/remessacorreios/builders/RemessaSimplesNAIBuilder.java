...
public class RemessaSimplesNAIBuilder implements IBuilderRemessa {
	private DadosDocumento dadosDocumento;
	private CustomConnection customConnection;
	private IAitMovimentoService aitMovimentoServices;
	
	public RemessaSimplesNAIBuilder() throws Exception {
	...
	public void montarDadosRemessa(LoteImpressao loteImpressao, CustomConnection customConnection) throws Exception {
		...
		dadosDocumento.setNmDocumento("mob/nai_v2");
	...
	private List<DadosNotificacao> getDadosNotificacao(List<LoteImpressaoAitDTO> loteImpressaoAitDTOList) throws Exception {
		...
		for (LoteImpressaoAitDTO loteImpressaoAitDTO: loteImpressaoAitDTOList) {
			...
			this.dadosDocumento.setNmDocumento(selecionaNomeDocumento(dadosNotificacao));
	...
	private String selecionaNomeDocumento(DadosNotificacao dadosNotificacao) throws Exception {
		int tpEquipamento = dadosNotificacao.getTpEquipamento();
		int nrCodDetran = dadosNotificacao.getNrCodDetran();
		return new TipoEquipamentoFactory().getStrategy(tpEquipamento).getNomeRelatorio(nrCodDetran, this.dadosDocumento.getNmDocumento());
	}

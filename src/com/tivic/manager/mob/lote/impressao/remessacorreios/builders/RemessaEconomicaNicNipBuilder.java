...
public class RemessaEconomicaNicNipBuilder implements IBuilderRemessa {
	...
	public RemessaEconomicaNicNipBuilder() throws Exception {
	...
	public void montarDadosRemessa(LoteImpressao loteImpressao, CustomConnection customConnection) throws Exception { 
		...
		int tipoAr = Integer.parseInt(ParametroServices.getValorOfParametro("mob_impressao_tp_modelo_nip"));
		...
		dadosDocumento.setNmDocumento((tipoAr == TipoArDigitalEnum.AR_DIGITAL.getKey() ? "mob/nic_nip_remessa_economica_ar_digital": "mob/nic_nip_remessa_economica_ar_digital_2D"));
		dadosDocumento.setArquivoPostagem(new ArquivoPrevisaoPostagem(dadosDocumento, customConnection).gerar());
		dadosDocumento.setArquivoListaPostagem(new ArquivoListaPostagem(dadosDocumento, TipoLoteNotificacaoEnum.LOTE_NIC_NIP.getKey(), customConnection).gerar());
	}
	...
	private Search<DadosNotificacao> searchNip(SearchCriterios searchCriterios) throws Exception {
		Search<DadosNotificacao> search = new SearchBuilder<DadosNotificacao>("MOB_AIT A")
				.fields(  " A.cd_Ait, A.id_ait, A.nr_renainf, A.nr_controle, A.dt_infracao, A.dt_prazo_defesa, A.ds_observacao, A.nr_cpf_condutor, A.dt_afericao,"
						...
						+ " A.nm_condutor, A.nr_cnh_condutor, A.uf_cnh_condutor, A.ds_local_infracao, A.ds_ponto_referencia, A.vl_multa, A.vl_velocidade_permitida, "
	...
	private List<CorreiosEtiqueta> getEtiquetas(int cdLoteNotificacao, List<DadosNotificacao> dadosNotificacaoList) throws Exception {
		...
		for (int index = 0; dadosNotificacaoList.size() > index; index++) {
			DadosNotificacao dadosNotificacao = dadosNotificacaoList.get(index);
	        CorreiosEtiqueta correiosEtiqueta = correiosEtiquetasList.get(index);
	        new DadosNotificacaoBuilder(dadosNotificacao, correiosEtiqueta).build();
		}

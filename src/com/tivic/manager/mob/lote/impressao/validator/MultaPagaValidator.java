package com.tivic.manager.mob.lote.impressao.validator;

import java.util.List;

import com.tivic.manager.mob.Ait;
import com.tivic.manager.mob.AitMovimento;
import com.tivic.manager.mob.AitReportErrorException;
import com.tivic.manager.mob.TipoStatusEnum;
import com.tivic.manager.util.validator.Validator;
import com.tivic.sol.connection.CustomConnection;
import com.tivic.sol.search.Search;
import com.tivic.sol.search.SearchBuilder;
import com.tivic.sol.search.SearchCriterios;

public class MultaPagaValidator implements Validator<Ait>  {
	
	@Override
	public void validate(Ait object, CustomConnection customConnection) throws Exception {
		List<AitMovimento> listMovimento = searchMultaPaga(object).getList(AitMovimento.class);
		if (listMovimento.size() == 0) {
			return;
		}
		throw new AitReportErrorException("Já existe Multa Paga para esse AIT.");
	}
	
	private Search<AitMovimento> searchMultaPaga(Ait ait) throws Exception {
		Search<AitMovimento> search = new SearchBuilder<AitMovimento>("mob_ait_movimento A")
				.fields("A.*")
				.searchCriterios(criteriosMultaPaga(ait))
				.build();
		return search;
	}
	
	private SearchCriterios criteriosMultaPaga(Ait ait) {
		SearchCriterios searchCriterios = new SearchCriterios();
		searchCriterios.addCriteriosEqualInteger("A.cd_ait", ait.getCdAit(), true);
		searchCriterios.addCriteriosEqualInteger("A.tp_status", TipoStatusEnum.MULTA_PAGA.getKey() , true);
		return searchCriterios;
	}
}

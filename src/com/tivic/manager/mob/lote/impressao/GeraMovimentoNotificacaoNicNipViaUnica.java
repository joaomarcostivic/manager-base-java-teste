...
public class GeraMovimentoNotificacaoNicNipViaUnica implements IGerarMovimentoNotificacao {
	...
	public GeraMovimentoNotificacaoNicNipViaUnica() throws Exception {
	...
	public List<ServicoDetranDTO> gerarMovimentos(List<Ait> listAits, int cdUsuario) throws Exception {
		...
		try {
			...
			verificarEnvioNip(listAits.get(0), customConnection); 
	...
	private void verificarEnvioNip(Ait ait, CustomConnection customConnection) throws ValidacaoException, Exception {
		AitMovimento nipEnviada = this.aitMovimentoServices.getMovimentoTpStatus(ait.getCdAit(), TipoStatusEnum.NIP_ENVIADA.getKey());
	...
	private void verificarGeracaoMovimento(List<Ait> listAits) throws ValidacaoException {
	    if (listAits.isEmpty()) {
	        throw new ValidacaoException("Não é possível gerar a NIP.");
	    }
	}

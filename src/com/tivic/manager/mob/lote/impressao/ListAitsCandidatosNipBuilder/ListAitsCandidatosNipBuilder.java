package com.tivic.manager.mob.lote.impressao.ListAitsCandidatosNipBuilder;
...
import com.tivic.manager.mob.lote.impressao.IListAitsCandidatasNotificacao;
import com.tivic.manager.mob.lote.impressao.PrazosNIPEnum;
...
public class ListAitsCandidatosNipBuilder implements IListAitsCandidatasNotificacao{
	private CustomConnection customConnection;
	private List<Ait> aitList;
	private SearchCriterios searchCriterios;
	private IParametroService parametroService;
	
	public ListAitsCandidatosNipBuilder(SearchCriterios searchCriterios, CustomConnection customConnection) throws Exception {
	...
	private Search<Ait> buscarAitsCandidatas() throws Exception {
		Search<Ait> search = new SearchBuilder<Ait>("MOB_AIT A")
				...
				.additionalCriterias( "EXISTS ( SELECT B1.tp_status, B1.lg_enviado_detran from mob_ait_movimento B1 "
							   + "				 WHERE (B1.tp_status = " + TipoStatusEnum.FIM_PRAZO_DEFESA.getKey() + " "
							   + "						AND B1.lg_enviado_detran = " + TipoStatusEnum.ENVIADO_AO_DETRAN.getKey() +")"
							   + "				 AND B1.cd_ait = A.cd_ait ORDER BY B1.dt_movimento DESC "
							   + "				)"
							   + "		   AND NOT EXISTS( "
							   ...
							   + "				WHERE("
							   + "					tp_status IN("+ TipoStatusEnum.NIP_ENVIADA.getKey() + ", " + TipoStatusEnum.NOTIFICACAO_ADVERTENCIA.getKey() + ")"
							   + " 				)"
							   + "				AND B4.cd_ait = A.cd_ait"	
							   + "  	     )"
							   + "		     OR EXISTS ( "
							   + "		 		SELECT B5.tp_status from mob_ait_movimento B5"
							   + "				WHERE (B5.tp_status IN ("+ TipoStatusEnum.NIP_ENVIADA.getKey() + ", " + TipoStatusEnum.NOTIFICACAO_ADVERTENCIA.getKey() + "))"
							   ...
							   + "                      	AND B6.dt_movimento > ("
							   + "                          	SELECT B7.dt_movimento FROM mob_ait_movimento B7 WHERE B7.tp_status IN (" + TipoStatusEnum.NIP_ENVIADA.getKey() + ", " 
							   + 								TipoStatusEnum.NOTIFICACAO_ADVERTENCIA.getKey() + ")"
							   + "       						AND B7.cd_ait = A.cd_ait ORDER BY dt_movimento DESC limit 1 )"
							   + "							AND EXISTS ("
							   + " 								SELECT B8.dt_movimento FROM mob_ait_movimento B8 "
							   + "								WHERE ( B8.tp_status = " + TipoStatusEnum.FIM_PRAZO_DEFESA.getKey()
							   + "									AND B8.lg_enviado_detran = " + TipoLgEnviadoDetranEnum.REGISTRADO.getKey()							
							   + "									AND B8.dt_movimento > ("
							   + "										SELECT B9.dt_movimento FROM mob_ait_movimento B9 WHERE B9.tp_status = " + TipoStatusEnum.CANCELAMENTO_NIP.getKey() 
							   + "							          		AND B9.cd_ait = A.cd_ait ORDER BY dt_movimento DESC limit 1 )"
							   + "									) AND B8.cd_ait = A.cd_ait"
							   + "									ORDER BY B8.dt_movimento DESC limit 1"	
							   + "								) "		
							   ...
							   + "			WHERE "	
							   + "			( "
							   + "				J.tp_status = " + TipoStatusEnum.ADVERTENCIA_DEFESA_DEFERIDA.getKey() + " AND I.dt_infracao > '"+formatDatePrazoLegal()+"'"
							   + "				OR J.tp_status = " + TipoStatusEnum.NAI_ENVIADO.getKey()
							   ...
							   + " 				WHERE ( D.tp_documento = " + TipoLoteNotificacaoEnum.LOTE_NIP.getKey() 
							   ...
							   + "						WHERE E2.cd_ait = A.cd_ait AND D1.tp_documento = " + TipoLoteNotificacaoEnum.LOTE_NIP.getKey() 
	...
	private String formatDatePrazoLegal() throws Exception {
		...
		if(inicioPrazoLegal == null || Objects.equals(inicioPrazoLegal, "0")) {
			throw new Exception("O parâmetro MOB_INFORMACOES_ADICIONAIS_DATA_INICIO_EMISSAO_NP é obrigatório e não foi informado.");
		}

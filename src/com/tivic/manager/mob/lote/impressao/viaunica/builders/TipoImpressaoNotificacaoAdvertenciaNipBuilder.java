...
public class TipoImpressaoNotificacaoAdvertenciaNipBuilder {
	private TipoImpressaoNotificacao tipoImpressaoNotificacao;
	private IAitMovimentoService aitMovimentoServices;

	public TipoImpressaoNotificacaoAdvertenciaNipBuilder(int cdAit, int tpNotificao, int cdUsuario) throws Exception {
	...
	private void obterDadosNotificacao(int cdAit, int tpNotificao, int cdUsuario) throws Exception {
		...
		AitMovimento aitMovimentoAdvertencia = this.aitMovimentoServices.getMovimentoTpStatus(cdAit, TipoStatusEnum.NOTIFICACAO_ADVERTENCIA.getKey());
		...
		this.tipoImpressaoNotificacao.setMovimentoAdvertencia(aitMovimentoAdvertencia.getCdMovimento() > 0 
				&& aitMovimentoAdvertencia.getLgEnviadoDetran() != TipoLgEnviadoDetranEnum.REGISTRO_CANCELADO.getKey()
				&& aitMovimentoAdvertencia.getLgEnviadoDetran() != TipoLgEnviadoDetranEnum.NAO_ENVIAR_CANCELADO.getKey());
	}
	...
	private Search<LoteImpressao> searchLote(int cdAit) throws Exception {
		Search<LoteImpressao> search = new SearchBuilder<LoteImpressao>("mob_lote_impressao A")
				.fields("A.cd_lote_impressao, A.tp_documento, B.cd_ait")
	...
	private SearchCriterios searchCriteriosLote(int cdAit) {
		...
		search.addCriterios("A.tp_documento", String.valueOf(TipoLoteNotificacaoEnum.LOTE_NIP.getKey()) + ", " +
	    String.valueOf(TipoLoteNotificacaoEnum.LOTE_NIC_NIP.getKey()), ItemComparator.IN, Types.INTEGER);

...
import com.tivic.manager.mob.lote.impressao.viaunica.nic.validacao.INicNipViaUnicaValidador;
import com.tivic.manager.mob.lote.impressao.viaunica.nic.validacao.NicNipViaUnicaValidador;
import com.tivic.sol.cdi.BeansFactory;
import com.tivic.sol.connection.CustomConnection;
import com.tivic.sol.search.SearchCriterios;

public class GeraNicNipViaUnica extends TipoImpressaoNotificacaoNPHandler {

	private ILoteNotificacaoService loteNotificacaoService;
	private IAitMovimentoService aitMovimentoService;
	private INicNipViaUnicaValidador nicNipViaUnicaValidador;
	
	public GeraNicNipViaUnica() throws Exception {
		...
		this.nicNipViaUnicaValidador = new NicNipViaUnicaValidador(); 
	}
	...
	public NipImpressaoDTO gerar(TipoImpressaoNotificacao tipoImpressaoNotificacao, CustomConnection customConnection) throws Exception {
		...
		if (nicNipViaUnicaValidador.validate(tipoImpressaoNotificacao, customConnection)) {
			List<Ait> aitList = new ListAitsCandidatosNipFactory()
					.getEstrategiaListAits(TipoLoteNotificacaoEnum.LOTE_NIC_NIP_VIA_UNICA.getKey(), searchNotificacaoViaUnica(tipoImpressaoNotificacao.getCdAit()), customConnection)
					.build();
			finalizarPrazoDefesa(aitList, tipoImpressaoNotificacao.getCdUsuario());
			new GerarMovimentoNotificacaoFactory().strategy(TipoLoteNotificacaoEnum.LOTE_NIC_NIP_VIA_UNICA.getKey())
			...
			LoteImpressao loteNotificacaoNipViaUnica = this.loteNotificacaoService.gerarLoteNotificacaoAitViaUnica(
					aitList, tipoImpressaoNotificacao.getCdUsuario(), TipoLoteNotificacaoEnum.LOTE_NIC_NIP.getKey());
	...
	private AitMovimento buscarFimPrazoDefesa (int cdAit, int tpStatus) throws Exception {
		AitMovimento aitMovimentoCancelamento= this.aitMovimentoService.getMovimentoTpStatus(cdAit, new CodigoCancelamentoMovimentoMap().get(tpStatus));
		AitMovimento aitMovimentoFimPrazoDefesa = this.aitMovimentoService.getMovimentoTpStatus(cdAit, TipoStatusEnum.FIM_PRAZO_DEFESA.getKey());

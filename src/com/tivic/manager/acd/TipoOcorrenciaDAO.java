...
public class TipoOcorrenciaDAO{
	...
	public static int insert(TipoOcorrencia objeto, Connection connect){
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO acd_tipo_ocorrencia (cd_tipo_ocorrencia) VALUES (?)");
	...
	public static int update(TipoOcorrencia objeto, int cdTipoOcorrenciaOld, Connection connect){
		...
		try {
			...
			if (objetoTemp == null)
				pstmt = connect.prepareStatement("INSERT INTO acd_tipo_ocorrencia (cd_tipo_ocorrencia) VALUES (?)");
			else
				pstmt = connect.prepareStatement("UPDATE acd_tipo_ocorrencia SET cd_tipo_ocorrencia=? WHERE cd_tipo_ocorrencia=?");
			pstmt.setInt(1,objeto.getCdTipoOcorrencia());
			if (objetoTemp != null) {
	...
	public static int delete(int cdTipoOcorrencia, Connection connect){
		...
		try {
			...
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM acd_tipo_ocorrencia WHERE cd_tipo_ocorrencia=?");
	...
	public static TipoOcorrencia get(int cdTipoOcorrencia, Connection connect){
		...
		ResultSet rs;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM acd_tipo_ocorrencia A, grl_tipo_ocorrencia B WHERE A.cd_tipo_ocorrencia=B.cd_tipo_ocorrencia AND A.cd_tipo_ocorrencia=?");
			pstmt.setInt(1, cdTipoOcorrencia);
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM acd_tipo_ocorrencia");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM acd_tipo_ocorrencia", criterios, true, connect!=null ? connect : Conexao.conectar(), connect==null);
	}

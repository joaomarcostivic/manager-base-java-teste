...
public class LocalServices {
	...
	public static ResultSetMap getAllByTipo(int cdTipoLocal) {
		return getAllByTipo(cdTipoLocal, null);
	}

	public static ResultSetMap getAllByTipo(int cdTipoLocal, Connection connect) {
		boolean isConnectionNull = connect==null;
		if (isConnectionNull)
			connect = Conexao.conectar();
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT A.*, B.nm_tipo_local" + 
											" FROM AGD_LOCAL A" +
											" LEFT OUTER JOIN AGD_TIPO_LOCAL B ON (A.cd_tipo_local = B.cd_tipo_local)" +
											" WHERE A.cd_tipo_local = " + cdTipoLocal);
				return new ResultSetMap(pstmt.executeQuery());
		}
		catch(SQLException sqlExpt) {
			sqlExpt.printStackTrace(System.out);
			System.err.println("Erro! LocalServices.getAllByTipo: " + sqlExpt);
			return null;
		}
		catch(Exception e) {
			e.printStackTrace(System.out);
			System.err.println("Erro! LocalServices.getAllByTipo: " + e);
			return null;
		}
		finally {
			if (isConnectionNull)

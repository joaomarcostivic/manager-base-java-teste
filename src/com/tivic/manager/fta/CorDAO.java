...
public class CorDAO{
	...
	public static int insert(Cor objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("fta_cor", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO fta_cor (cd_cor,"+
			                                  "nm_cor) VALUES (?, ?)");
			pstmt.setInt(1, objeto.getCdCor());
			pstmt.setString(2,objeto.getNmCor());
			pstmt.executeUpdate();
			return objeto.getCdCor();
		}
	...
	public static int update(Cor objeto, int cdCorOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE fta_cor SET cd_cor=?,"+
												      		   "nm_cor=? WHERE cd_cor=?");
			pstmt.setInt(1,objeto.getCdCor());
			pstmt.setString(2,objeto.getNmCor());
	...
	public static int delete(int cdCor, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM fta_cor WHERE cd_cor=?");

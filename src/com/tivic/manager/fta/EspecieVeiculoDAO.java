...
public class EspecieVeiculoDAO{
	...
	public static int insert(EspecieVeiculo objeto, Connection connect){
		...
		try {
			...
			int code = Conexao.getSequenceCode("fta_especie_veiculo", connect);
			if (code <= 0) {
				if (isConnectionNull)
			...
			PreparedStatement pstmt = connect.prepareStatement("INSERT INTO fta_especie_veiculo (cd_especie,"+
			                                  "ds_especie) VALUES (?, ?)");
			pstmt.setInt(1, objeto.getCdEspecie());
			pstmt.setString(2,objeto.getDsEspecie());
			pstmt.executeUpdate();
			return objeto.getCdEspecie();
		}
	...
	public static int update(EspecieVeiculo objeto, int cdEspecieOld, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("UPDATE fta_especie_veiculo SET cd_especie=?,"+
	...
	public static int delete(int cdEspecie, Connection connect){
		...
		try {
			if (isConnectionNull)
				connect = Conexao.conectar();
			PreparedStatement pstmt = connect.prepareStatement("DELETE FROM fta_especie_veiculo WHERE cd_especie=?");
	...
	public static ResultSetMap getAll(Connection connect) {
		...
		PreparedStatement pstmt;
		try {
			pstmt = connect.prepareStatement("SELECT * FROM fta_especie_veiculo");
	...
	public static ResultSetMap find(ArrayList<ItemComparator> criterios, Connection connect) {
		return Search.find("SELECT * FROM fta_especie_veiculo", criterios, true, connect!=null ? connect : Conexao.conectar(), connect==null);
	}
